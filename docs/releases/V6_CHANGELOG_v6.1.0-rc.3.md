...
_This changelog describes the cumulative `v6.1.0-rc.3` release candidate
compared with stable `v6.0.5`. It supersedes `v6.1.0-rc.2`._
...
- New Patrol findings can route to the configured alert notification channels,
  with settings exposed in the API and the Patrol settings page.
- The Docker thresholds page exposes the container update-alert delay with an
  explicit off state.
- Relay is discoverable from alert configuration, the Assistant surfaces at
  first AI setup, and the external-agent (MCP) connector setup is findable
  from sidebar search.
...
- Host agent install tokens now carry the command-execution scope the
  Add Pulse Agent checkbox asks for, with first-use command-channel binding
  and regeneration when the choice changes.
- ZFS pool membership resolves `nvme-eui.<hex>` and namespace-suffixed by-id
  member references on both the server and host-agent matchers.
- TrueNAS storage reads the API shapes TrueNAS actually serves, so datasets,
  disks, and pool details report real state.
- Disable-all Docker alert toggles clear active container update alerts.
- Container update refusals surface their reason up front, including the
  agent-too-old case.
- Manual Patrol runs only report a failed start after a status refresh
  confirms no run is in progress.
- In-app updates select releases by highest version, not GitHub list order,
  and malformed release notes fail closed.
- Webhook and security dialers try every permitted resolved IP.
- Approved-action dispatch survives client disconnects, and the Docker agent
  collect cycle is watchdog-bounded.
- The zero-connected Add infrastructure calls to action are distinctly named,
  and telemetry pings are suppressed in mock mode.
