# Pulse v6.4.5-beta.3 Release Notes
...
published `v6.4.5-beta.1`. The `v6.4.5-beta.2` preparation did not complete
publication. Beta.3 carries every change from the `v6.4.2` packet that was never
published, together with the same corrections as beta.2. It is not a stable
release. Keep stable `v6.4.1` available for rollback.
