...
- Monitoring state snapshots now use the same synchronization boundary as
  startup and reset writes.
- The correction removes a race found during release validation. The first
  beta.3 candidate was rejected and was not published.

## Verification scope

- Focused Go race checks cover canonical metrics targets.
- Focused monitoring race checks cover concurrent startup, snapshot reads, and
  mock-state reset.
...
- The retained beta.2 metrics-handler advisory result remains open before RC.
- Paired candidate measurements repeatedly show slower low-level route-segment
  normalization with unchanged allocations. Focused full History middleware
  measurements did not reproduce a slowdown, and no installed or user-facing
  regression is established. Preserve both the adverse and bounding evidence
  for RC disposition.
