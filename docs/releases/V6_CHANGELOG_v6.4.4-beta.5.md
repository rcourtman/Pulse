...
Beta.3 and beta.4 did not complete publication. Beta.5 fixes that History
checkpoint forward with metrics-statistics connection isolation while retaining
the alert, recovery, API, host, update, identity, reconnect, and accessibility
repairs from beta.2. It also carries the complete `v6.4.2` change set through
the published preview line.
