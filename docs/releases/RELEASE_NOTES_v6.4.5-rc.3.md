# Pulse v6.4.5-rc.3 Release Notes

This release candidate follows `v6.4.5-rc.2` and gathers the accumulated monitoring, notification, agent and installation reliability repairs since stable `v6.4.1` into one build intended to become the next stable release. It carries the repairs from `v6.4.5-rc.2` and adds AI knowledge-store save serialization, PBS History host-target retention, and provider-hosted client upgrade and isolation repairs. Preview users can opt in to confirm the fixes before stable promotion. Keep stable `v6.4.1` available for rollback.
...
- **PBS History keeps its identity** - Linked rows correlate only with proven identity. Datastores stay distinct across refreshes, and History remains on the PBS host target when its connection uses an IP or alias (#1723).
