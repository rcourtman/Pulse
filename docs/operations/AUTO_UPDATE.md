...
**Release channel note:** the systemd timer script tracks GitHub `releases/latest` (stable). The internal `rc` channel only affects manual and in-app pre-release selection. `stable` is the only recommended channel for paid or production environments.
