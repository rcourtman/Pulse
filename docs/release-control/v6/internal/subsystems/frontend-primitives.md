...
ConfigMaps, and ServiceAccounts, must map to the existing reporting `k8s`
transport token at this edge rather than widening the metric-report picker into
platform-object inventory. The shell must not re-accumulate license
