...
1. Update this contract when agent lifecycle ownership changes. Routes added under the shared `internal/api/` extension point that are clearly outside lifecycle ownership (for example `POST /api/ai/patrol/preflight` and the `patrol_preflight` snapshot field added to `/api/settings/ai`, both owned by ai-runtime) do not extend this subsystem's contract; they live in their owning subsystem.
