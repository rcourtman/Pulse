...
Those same governed release workflows now also own the operator-facing wording
for that promotion metadata. Human-visible workflow inputs, summaries, and
error messages must describe the path as a prerelease or preview flow rather
than implying a near-ready release candidate, while the machine-owned channel
and lineage identifiers such as `rc`, `rc-to-ga-*`, and `v6.0.0-rc.1` remain
the canonical internal keys.
