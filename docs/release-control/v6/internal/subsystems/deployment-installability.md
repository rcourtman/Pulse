...
The active prerelease `v6.4.5-rc.3` cut sets the repo-root `VERSION`, repo-root `docker-compose.yml` image default, `scripts/install-docker.sh` fallback, and Helm chart release metadata to the same `6.4.5-rc.3` release version.
It follows `v6.4.5-rc.2` on the published `v6.4.5` candidate line.
...
For the active prerelease `v6.4.5-rc.3` cut, the repo-root compose default and `scripts/install-docker.sh` fallback must both pin `6.4.5-rc.3` until the next governed stable cut moves them forward.
