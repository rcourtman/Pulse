...
3. Optimize hot paths only when backed by benchmarks or proven query issues
   ...
   framing, and partial responses must preserve the identity representation
   whose byte positions `Content-Range` describes. Once a body-permitted full
   response advertises gzip, an explicit empty body must still emit a valid
   empty gzip member. Regression proof lives in
...
10. Extend workload row contract and per-row hot-path derivations through `frontend-modern/src/components/Workloads/guestRowModel.tsx` and `frontend-modern/src/components/Workloads/useGuestRowState.ts`, and extend tooltip-backed row cell presentation through `frontend-modern/src/components/Workloads/GuestRowCells.tsx`, rather than rebuilding column metadata, row identity, cell tooltips, or anomaly correlation inside `frontend-modern/src/components/Workloads/GuestRow.tsx`
    ...
    listeners, or render-time column discovery to the Workloads hot path.
...
informational responses defer negotiation to the final response and
204/205/206/304, SSE, already encoded, small declared, and binary responses
stay on their identity paths. Partial content retains the identity bytes that
its `Content-Range` describes.
