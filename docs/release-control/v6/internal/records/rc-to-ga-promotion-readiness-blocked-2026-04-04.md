...
3. The governed release profile in `docs/release-control/control_plane.json`
   ...
   `pulse/v6-release`.
4. The active control-plane target is still `v6-rc-cut`, not
   `v6-ga-promotion`.
5. The active local `pulse/v6-release` branch currently reports `VERSION=6.0.0-dev`, so the
...
1. Promote the active target from `v6-rc-cut` to
