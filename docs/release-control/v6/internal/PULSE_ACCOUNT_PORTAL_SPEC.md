...
Status: IMPLEMENTED (the `L17` RC floor is delivered: `internal/cloudcp/portal` account shell with `GET /api/portal/bootstrap`, plus the `pulse-pro/landing-page` utility pages; `status.json` lane `L17` is `bounded-residual`, and the post-lane follow-on list below is tracked only as lane-followup `account-commercial-transition-external-proof`, not a current build signal). Last reviewed 2026-09-01.
