...
func TestCurrentSupportPrereleasePacketTracksInstallMetadata(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, releaseNotesPath,
		...
		"## Highlights",
		"stable `v"+previous+"`",
		"supersedes `v6.2.0-rc.7`",
		"aligns the Go runtime memory limit",
		"routes now share one session-administrator model",
		"TrueNAS completed init containers no longer create permanent critical alerts",
		"Large threshold sections and expanded infrastructure tables remain usable",
		"Product telemetry now measures content-free adoption counts",
		"every successfully read WebSocket frame as client liveness",
		"Hardware-related fix claims now require version-bound live-runtime proof",
	...
	assertFileContainsAllNormalized(t, changelogPath,
		...
		"This changelog describes the changes since `v6.2.0-rc.7`",
		"Cgroup-aware Go memory-limit configuration",
		"Accepted any inbound WebSocket frame as proof of liveness",
		"Matched cross-site auto-registration to canonical Proxmox identity",
		"Stopped completed TrueNAS init containers from creating permanent critical incidents",
		"Corrected authorization and capability parity",
