...
func TestCurrentSupportPrereleasePacketTracksInstallMetadata(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, releaseNotesPath,
		...
		"## Highlights",
		"stable `v"+previous+"`",
		"supersedes `v6.2.0-rc.3`",
		"A working rootful Docker daemon now outranks any rootless socket",
		"transient socket-activated rootless Podman socket",
		"averaged across the report interval instead of a blocking one-second spot sample",
		"mostly-idle VMs stop reporting inflated CPU",
		"checks for a live system Docker daemon before it globs",
		"explicit `--enable-docker` runs alike",
		"reported as docker and keeps Swarm collection enabled",
		"re-runs runtime discovery after three consecutive daemon-unavailable collects",
		"busy/total delta of the cumulative CPU time counters between collections",
		"counter resets such as a reboot or a live migration",
		"`CPUUsagePercent` keeps its meaning in the host report schema",
	...
	assertFileContainsAllNormalized(t, changelogPath,
		...
		"This changelog describes the changes since `v6.2.0-rc.3`",
		"Installer container-runtime discovery prefers a live rootful Docker daemon over any rootless socket",
		"`system_docker_runtime_is_active` gates rootless discovery",
		"strips `DOCKER_HOST` and `CONTAINER_HOST`",
		"podman runtime preference as an ordering hint rather than an identity override",
		"swaps the connection behind a swappable client",
		"averaged across the report interval from the cumulative CPU time counters",
		"delayed every collect by a second",
