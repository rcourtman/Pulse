...
func TestCurrentSupportPrereleasePacketTracksInstallMetadata(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, releaseNotesPath,
		...
		"Live ntfy alert delivery now preserves the configured title, priority, and tags",
		"Saving notification settings no longer drops provider incidents or resets acknowledgement state",
		"disabling grouping now delivers queued alerts individually",
		"Grouped provider notifications describe every alert in the group",
		"Tenant offboarding and server shutdown now release per-tenant resource-store handles",
		"Resource APIs reuse per-generation raw and presentation lists",
		"Notification regression suites cover live ntfy metadata",
	...
	assertFileContainsAllNormalized(t, changelogPath,
		...
		"Shared per-generation resource lists across API requests",
		"Cached mock unified views against fixture versions",
		"Released per-tenant resource-store handles",
		"Preserved configured ntfy title, priority, and tags",
		"Kept provider incidents and acknowledged state intact",
		"Honored disabled alert grouping",
