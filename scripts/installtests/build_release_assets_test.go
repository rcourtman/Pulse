...
func TestCurrentPrereleasePacketTracksInstallMetadata(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, releaseNotesPath,
		...
		"## Before you upgrade",
		"## Known issues",
		"Clearer notification evidence",
		"More reliable alert lifecycles",
		"API-token deletion now persists the reduced inventory atomically",
		"Filesystem history drawers again show loading progress",
		"Pulse Mobile does not consume the changed alert transition internals",
		"not Authenticode-signed",
		"Unknown Publisher warning",
	)
	assertFileContainsAllNormalized(t, changelogPath,
		...
		"The append-only alert event log records lifecycle transitions and notification decisions",
		"Canonical metric, lifecycle, and stateful alert families use the deterministic reducer core",
		"API-token deletion snapshots the full inventory",
		"Canonically keyed metric alerts resolve through their canonical identity",
		"Stateful manual clears preserve recent-resolution and acknowledgement retention",
