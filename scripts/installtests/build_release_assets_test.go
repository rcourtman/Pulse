...
func TestCurrentStablePatchReleasePacketTracksInstallMetadata(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, releaseNotesPath,
		...
		"Windows Unified Agent binaries must be Authenticode-signed for `v"+version+"`",
		"no unsigned Windows owner exception is authorized for this version",
		"`no-mobile-impact`",
	...
	assertFileContainsAllNormalized(t, changelogPath,
		...
		"Windows signing decision: Authenticode signing is required for `v"+version+"`",
		"no unsigned Windows owner exception is authorized for this version",
