...
func TestCurrentPrereleasePacketTracksInstallMetadata(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, releaseNotesPath,
		...
		"## Before you upgrade",
		"## Known issues",
		"keyed per-client deltas",
		"Delivery warnings now clear",
		"QEMU guest agent",
		"Restored Proxmox network details",
		"Pulse Mobile does not consume the changed browser resource stream",
		"not Authenticode-signed",
		"Unknown Publisher warning",
	)
	assertFileContainsAllNormalized(t, changelogPath,
		...
		"Resources, connected-infrastructure entries, and active alerts use per-client keyed deltas",
		"REST state recovery is isolated from the websocket baseline",
		"Notification queue health is reconciled immediately",
		"Proxmox VM/LXC lifecycle actions remain available through the governed action path",
		"Proxmox node network details render again",
		"Backup recovery data retains the correct source attribution",
