...
func TestInstallDockerProofTracksSupportPrereleaseContract(t *testing.T) {
	...
	assertFileContainsAllNormalized(t, repoFile("docs", "release-control", "v6", "internal", "subsystems", "deployment-installability.md"),
		...
		"is a monitoring-correctness, deployment-safety, security, and responsive-interface cut that supersedes `v6.2.0-rc.8`: it corrects alert delivery and infrastructure state, hardens agent install and update behavior, tightens authorization and dependency boundaries, and keeps operational controls usable across narrow layouts",
