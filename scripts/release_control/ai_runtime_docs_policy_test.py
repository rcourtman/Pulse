...
class AIRuntimeDocsPolicyTest(unittest.TestCase):
    ...
    def test_public_ai_docs_define_local_subscription_agent_safety_boundary(self) -> None:
        ...
        self.assertIn("Pulse audits Claude's buffered event stream", normalized_content)
        self.assertIn("accepts only the first call whose name was explicitly offered", normalized_content)
        self.assertIn("undeclared local tool attempts fail closed", normalized_content)
