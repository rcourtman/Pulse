...
A canonical runtime commit whose required contract or proof lands in an
immediately following commit cannot satisfy the per-commit completion guard on
its own. This module accepts a pair only when the registry records the exact
incomplete and completion commits and the completion tree itself passes the
canonical completion guard for the pair's combined changed files. The registry
is the authority; this module is the mechanical check.
"""
