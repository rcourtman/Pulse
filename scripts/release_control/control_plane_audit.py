...
def current_path_kinds(*, staged: bool) -> dict[str, str]:
    ...
    kinds: dict[str, str] = {
        AGENT_VALUES_DOC_REL: _path_kind(AGENT_VALUES_DOC_REL, expected_kind="file", staged=staged),
...
def audit_control_plane_payload(
    ...
    *,
    ...
    values_kind = path_kinds.get(AGENT_VALUES_DOC_REL, "missing")
    if values_kind != "file":
        errors.append(
            f"agent values doc {AGENT_VALUES_DOC_REL!r} must exist as a tracked file, found {values_kind!r}"
        )
    if doc_kind != "file":
        errors.append(
