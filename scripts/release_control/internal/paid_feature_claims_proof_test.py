...
class PaidFeatureClaimsProofTest(unittest.TestCase):
    ...
    def test_build_command_specs_cover_paid_claim_layers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(specs[5].cwd, str(frontend_dir.resolve()))
            self.assertEqual(specs[6].cwd, str(pulse_pro_license_server_dir.resolve()))
            self.assertEqual(specs[7].cwd, str(pulse_pro_relay_server_dir.resolve()))
            self.assertEqual(specs[8].cwd, str(pulse_enterprise_dir.resolve()))
            ...
            self.assertIn("pulse-frontend-free-first-commercial-boundary", names)
