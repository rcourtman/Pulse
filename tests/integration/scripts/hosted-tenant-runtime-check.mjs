...
import { assertHostedTenantRuntimeExists } from './hosted-tenant-runtime.mjs';
...
function usage(message) {
  ...
  console.error(
    'usage: node ./tests/integration/scripts/hosted-tenant-runtime-check.mjs --tenant-id <id> [--cloud-host <user@host>]',
  );
  process.exit(1);
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  const status = assertHostedTenantRuntimeExists(args.cloudHost, args.tenantId).trim();
