...
func TestV5DataDir_SessionLegacyMapFormat(t *testing.T) {
	...
	// Trigger a save to verify the on-disk format is migrated to hashed keys.
	// CreateSession implicitly calls saveUnsafe().
	store.CreateSession("trigger-save-token", time.Hour, "test", "127.0.0.1", "saveuser")

	// Re-read the file and verify it's now in the hashed (array) format, not legacy map format.
	...
	assert.GreaterOrEqual(t, len(hashedFormat), 2, "saved file must contain the migrated legacy session + the new session")
}
