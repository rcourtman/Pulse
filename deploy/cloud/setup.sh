...
validate_env_file() {
  ...
  local missing=()
  local k v
  for k in DOMAIN ACME_EMAIL CF_DNS_API_TOKEN CP_ENV TRAEFIK_IMAGE CONTROL_PLANE_IMAGE CP_ADMIN_KEY CP_PULSE_IMAGE CP_TRIAL_SIGNUP_PRICE_ID CP_TRIAL_ACTIVATION_PRIVATE_KEY CP_ALLOW_DOCKERLESS_PROVISIONING CP_REQUIRE_EMAIL_PROVIDER RESEND_API_KEY PULSE_EMAIL_FROM STRIPE_WEBHOOK_SECRET STRIPE_API_KEY; do
