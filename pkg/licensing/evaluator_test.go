...
func TestEvaluator_GetLimit(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "finds_limit",
			source:     mockSource{limits: map[string]int64{"max_monitored_systems": 100}},
			key:        "max_monitored_systems",
			wantValue:  100,
			wantExists: true,
		},
		...
		},
		{
			...
			source:     mockSource{limits: map[string]int64{"max_monitored_systems": 0}},
			key:        "max_monitored_systems",
			wantValue:  0,
			wantExists: true,
		},
...
func TestEvaluator_NilSource_ReturnsDefaults(t *testing.T) {
	...
	if got, _ := e.GetLimit("max_monitored_systems"); got != 0 {
		t.Errorf("GetLimit() on nil = %v, want 0", got)
	}
	if got := e.CheckLimit("max_monitored_systems", 50); got != LimitAllowed {
