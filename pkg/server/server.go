...
func Run(ctx context.Context, version string) (runErr error) {
	...
	// WebSocket state ownership is tenant-aware by construction, including default-org state.
	wsHub.SetStateGetter(func(orgID string) interface{} {
		...
		if mtMonitor == nil {
			...
			return models.EmptyStateFrontend()
		}
	...
	telemetryCfg := telemetry.Config{
		...
		GetSnapshot: func() telemetry.Snapshot {
			...
			if router != nil {
				...
				router.ApplyWorkloadHistoryTelemetrySnapshot(&snap, now)
			}
			...
			applyPulseIntelligenceTelemetrySnapshot(&snap, telemetryPersistence, currentCfg, actionSnapshot, now)
...
func ShouldAutoImport() bool {
	configPath := config.ResolveRuntimeDataDir("")
