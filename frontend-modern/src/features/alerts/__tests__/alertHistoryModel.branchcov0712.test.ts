...
// ---------------------------------------------------------------------------
// buildSelectedBucketDetails — null guard, the `0`-vs-null distinction,
// exact {rangeLabel,start,end} shapes for every formatAlertBucketRange arm
// (same-day, cross-day, cross-year), bucketSize 0, locale passthrough, and
// the no-bounds-guard behaviour for out-of-range/negative indices.
// ---------------------------------------------------------------------------
...
describe('buildSelectedBucketDetails', () => {
  function makeTrends(
    bucketTimes: number[],
    bucketSize: number,
    overrides: Partial<AlertTrendSeries> = {},
  ): AlertTrendSeries {
    return {
      buckets: bucketTimes.map(() => 0),
      max: 1,
      bucketSize,
      bucketTimes,
      rangeStart: bucketTimes[0] ?? 0,
      rangeHours: bucketTimes.length * bucketSize,
      ...overrides,
    };
  }
  ...
  it('returns exactly null when selectedBarIndex is null', () => {
    const trends = makeTrends([Date.UTC(2026, 2, 22, 12, 0, 0)], 1);
    expect(buildSelectedBucketDetails(null, trends, 'en-US')).toBeNull();
  });

  it('treats index 0 as a valid bucket (not falsy-null) and returns a details object', () => {
    const start = Date.UTC(2026, 2, 22, 12, 0, 0);
    const trends = makeTrends([start], 1);
    const details = buildSelectedBucketDetails(0, trends, 'en-US');
    expect(details).not.toBeNull();
    expect(details!.start).toBe(start);
    expect(details!.end).toBe(start + MS_PER_HOUR);
  });

  // --- exact full object shape for every formatting arm -------------------

  it('returns the exact {rangeLabel,start,end} object for a same-day bucket', () => {
    const start = Date.UTC(2026, 2, 22, 12, 0, 0);
    const trends = makeTrends([start], 1);
    expect(buildSelectedBucketDetails(0, trends, 'en-US')).toStrictEqual({
      rangeLabel: 'Mar 22, 12:00 PM – 1:00 PM',
      start,
      end: start + MS_PER_HOUR,
    });
  });

  it('produces a cross-day rangeLabel with the arrow separator', () => {
    const start = Date.UTC(2026, 2, 22, 22, 0, 0);
    const trends = makeTrends([start], 6);
    expect(buildSelectedBucketDetails(0, trends, 'en-US')).toStrictEqual({
      rangeLabel: 'Mar 22, 10:00 PM → Mar 23, 2026, 4:00 AM',
      start,
      end: start + 6 * MS_PER_HOUR,
    });
  });

  it('produces a cross-year rangeLabel that adds the year to the start segment', () => {
    // Bucket spans New Year's Eve midnight → New Year's Day, crossing both day
    // and year boundaries. formatAlertBucketRange must include the year on the
    // start segment because start.getFullYear() !== end.getFullYear().
    const start = Date.UTC(2025, 11, 31, 22, 0, 0);
    const trends = makeTrends([start], 4);
    expect(buildSelectedBucketDetails(0, trends, 'en-US')).toStrictEqual({
      rangeLabel: 'Dec 31, 2025, 10:00 PM → Jan 1, 2026, 2:00 AM',
      start,
      end: start + 4 * MS_PER_HOUR,
    });
  });

  // --- bucketSize edge -----------------------------------------------------

  it('yields end === start when bucketSize is 0', () => {
    const start = Date.UTC(2026, 2, 22, 12, 0, 0);
    const trends = makeTrends([start], 0);
    const details = buildSelectedBucketDetails(0, trends, 'en-US');
    expect(details).not.toBeNull();
    expect(details!.start).toBe(start);
    expect(details!.end).toBe(start);
    expect(details!.rangeLabel).toBe('Mar 22, 12:00 PM – 12:00 PM');
  });

  // --- locale passthrough --------------------------------------------------

  it('routes distinct locale formatting through to formatAlertBucketRange', () => {
    const start = Date.UTC(2026, 2, 22, 12, 0, 0);
    const trends = makeTrends([start], 1);
    const us = buildSelectedBucketDetails(0, trends, 'en-US');
    const gb = buildSelectedBucketDetails(0, trends, 'en-GB');
    expect(us!.rangeLabel).toBe('Mar 22, 12:00 PM – 1:00 PM');
    expect(gb!.rangeLabel).toBe('22 Mar, 12:00 – 13:00');
    expect(us!.rangeLabel).not.toBe(gb!.rangeLabel);
    // Numeric fields are locale-independent.
    expect(us!.start).toBe(gb!.start);
    expect(us!.end).toBe(gb!.end);
  });

  // --- multi-bucket index resolution --------------------------------------

  it('resolves a non-zero middle index against the correct bucketTimes entry', () => {
    const t0 = Date.UTC(2026, 2, 22, 0, 0, 0);
    const t1 = t0 + MS_PER_HOUR;
    const t2 = t0 + 2 * MS_PER_HOUR;
    const trends = makeTrends([t0, t1, t2], 1);
    const details = buildSelectedBucketDetails(2, trends, 'en-US');
    expect(details).not.toBeNull();
    expect(details!.start).toBe(t2);
    expect(details!.end).toBe(t2 + MS_PER_HOUR);
  });

  // --- no bounds guard: out-of-range / negative indices -------------------

  it('produces undefined start and NaN end for an out-of-bounds index (no guard)', () => {
    const start = Date.UTC(2026, 2, 22, 12, 0, 0);
    const trends = makeTrends([start], 1);
    const details = buildSelectedBucketDetails(99, trends, 'en-US');
    expect(details).not.toBeNull();
    expect(details!.start).toBeUndefined();
    expect(details!.end).toBeNaN();
    // formatAlertBucketRange receives (undefined, NaN) and renders "Invalid Date".
    expect(details!.rangeLabel).toBe('Invalid Date, Invalid Date → Invalid Date, Invalid Date');
  });

  it('produces undefined start and NaN end for a negative index', () => {
    const start = Date.UTC(2026, 2, 22, 12, 0, 0);
    const trends = makeTrends([start], 1);
    const details = buildSelectedBucketDetails(-1, trends, 'en-US');
    expect(details).not.toBeNull();
    expect(details!.start).toBeUndefined();
    expect(details!.end).toBeNaN();
  });
