...
describe('useAlertHistoryState', () => {
  ...
  it.each([true, false])('handles a pending fetch when clearing history succeeds=%s', async (succeeds) => {
    type History = Awaited<ReturnType<typeof AlertsAPI.getHistory>>;
    let resolveHistory!: (value: History) => void;
    const pendingHistory = new Promise<History>((resolve) => { resolveHistory = resolve; });
    ...
    const staleHistory = [{
      id: 'old-alert', type: 'cpu', level: 'warning',
      startTime: new Date().toISOString(), lastSeen: new Date().toISOString(),
      resourceId: 'resource-1', resourceName: 'db-01',
      message: 'CPU high', acknowledged: false,
    }] as History;
    const { result } = renderHook(() => useAlertHistoryState({
      activeAlerts: () => ({}), getResource: () => undefined, allResources: () => [],
    }));
    await waitFor(() => expect(AlertsAPI.getHistory).toHaveBeenCalledTimes(1));
