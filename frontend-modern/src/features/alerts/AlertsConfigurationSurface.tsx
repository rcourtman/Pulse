...
interface AlertsConfigurationSurfaceProps {
...
export function AlertsConfigurationSurface(props: AlertsConfigurationSurfaceProps) {
  ...
  const allGuests = createMemo(
    () => [...props.byType('vm'), ...props.byType('system-container'), ...props.byType('oci-container')],
    [],
    {
      equals: (prev, next) => {
        if (prev.length !== next.length) return false;
        return prev.every((current, index) => current.id === next[index].id && current.name === next[index].name);
      },
    },
  );
  ...
  createEffect(() => {
    ...
    const containerResources = [...props.byType('system-container'), ...props.byType('oci-container')];
    const storageResources = props.allResources().filter(
      (resource) => resource.type === 'storage' || resource.type === 'datastore',
    );
    ...
    try {
      ...
      if (config.guestDefaults) {
        setGuestDefaults({
          ...
          diskWrite: getTriggerValue(config.guestDefaults.diskWrite) ?? FACTORY_GUEST_DEFAULTS.diskWrite,
          networkIn: getTriggerValue(config.guestDefaults.networkIn) ?? FACTORY_GUEST_DEFAULTS.networkIn,
          networkOut: getTriggerValue(config.guestDefaults.networkOut) ?? FACTORY_GUEST_DEFAULTS.networkOut,
        });
      ...
      if (config.snapshotDefaults) {
        ...
        const rawWarning = config.snapshotDefaults.warningDays ?? FACTORY_SNAPSHOT_DEFAULTS.warningDays;
        const rawCritical = config.snapshotDefaults.criticalDays ?? FACTORY_SNAPSHOT_DEFAULTS.criticalDays;
        const safeCritical = Math.max(0, rawCritical);
        const normalizedWarning = Math.max(0, rawWarning);
        const warningDays =
          safeCritical > 0 && normalizedWarning > safeCritical ? safeCritical : normalizedWarning;
    ...
    <>
      ...
      <Show when={props.activeTab() === 'thresholds'}>
        <ThresholdsTab
          ...
          storage={props.allResources().filter(
            (resource) => resource.type === 'storage' || resource.type === 'datastore',
          )}
