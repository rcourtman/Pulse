...
import type { NormalizedHealth, StorageCategory, StorageRecord } from '@/features/storageBackups/models';
import {
...
// ===========================================================================
// Supplemental branch coverage for the four in-scope functions
// (isCephClusterPoolStorageRecord, getRecordPoolDetail, getRecordStatusDetail,
//  consolidateCephClusterPoolRecords). These cases reach the `(details || {})`
// right operands that the cases above do not actually exercise:
//  * isCephClusterPoolStorageRecord L21 — only reachable with nullish details
//    AND a 'ceph' type, which forces the type to derive from `category`.
//  * consolidateCephClusterPoolRecords L92 survivor spread — requires a
//    mounting sibling whose `details` are nullish (all mounts above had details).
//  * (Indirectly) getRecordPoolDetail L36 right operand — the nullish-details
//    candidate below is evaluated by the sibling `find` before matching on name.
//  * getRecordStatusDetail L41 right operand is UNREACHABLE through the public
//    surface: a pool row is only classified as such when details.node ===
//    'cluster', so no pool can ever carry nullish details. Reported, not hit.
// ===========================================================================

describe('cephRecordPresentation.branchcov2 supplemental (in-scope functions)', () => {
  it('isCephClusterPoolStorageRecord: takes the `(details || {})` right operand when details is nullish but the type derives from category', () => {
    // The pre-existing "defensive arm" case passes a truthy details object, so
    // the `{}` fallback is never constructed. Here details is undefined AND the
    // type resolves to 'ceph' via category, so the L20 guard is passed and L21
    // actually evaluates `undefined || {}` → {}. L22 then reads {}.node
    // (undefined) !== 'cluster' → false.
    const record = makeRecord({
      details: undefined,
      category: 'ceph' as unknown as StorageCategory,
    });
    expect(isCephClusterPoolStorageRecord(record)).toBe(false);
  });

  it('consolidate: spreads `(record.details || {})` on the survivor when the mount has nullish details', () => {
    // The mounting sibling carries no `details`. It still qualifies as a ceph
    // sibling via the isCephStorageRecord fallback (capabilities includes
    // 'replication' and source.platform includes 'proxmox'), and is matched on
    // name after getRecordPoolDetail returns '' for the nullish details. On
    // lift, L92 evaluates `(record.details || {})` → {} and overlays only the
    // status key, proving the `{}` right operand is taken.
    const pool = makePoolRecord({
      id: 'pool-1',
      name: 'cephfs-data',
      health: 'warning',
    ...
    expect(survivor.health).toBe('warning');
    expect(survivor.statusLabel).toBe('degraded');
    // mount.details was nullish → {} spread → only the overlaid status survives.
    expect(survivor.details).toEqual({ status: 'degraded' });
    expect(survivor.issueSummary).toBe('Ceph reports pool cephfs-data degraded');
  });
});
