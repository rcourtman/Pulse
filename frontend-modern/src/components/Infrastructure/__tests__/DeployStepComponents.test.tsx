...
const { getAgentInstallCommandMock } = vi.hoisted(() => ({
...
describe('ResultsStep', () => {
  ...
  it('fetches the canonical backend install command for manual fallback', async () => {
    getAgentInstallCommandMock.mockResolvedValue({
      command: 'curl -fsSL https://pulse.example/install.sh | bash -s -- --url https://pulse.example --token token-123',
    });
    const wizard = createMockWizard({
      failedTargets: [makeTarget({ status: 'failed_retryable' })],
    });
    ...
    await waitFor(() => {
      expect(getAgentInstallCommandMock).toHaveBeenCalledWith({ type: 'pve', enableProxmox: true });
    });
    expect(screen.getByText(/curl -fsSL https:\/\/pulse\.example\/install\.sh/)).toBeInTheDocument();
  });

  it('surfaces canonical install-command response validation failures in manual fallback', async () => {
    getAgentInstallCommandMock.mockRejectedValue(
      new Error('Invalid agent install command response'),
    );
    const wizard = createMockWizard({
      failedTargets: [makeTarget({ status: 'failed_retryable' })],
    });
    ...
    await waitFor(() => {
      expect(screen.getByRole('alert')).toHaveTextContent(
        'Invalid agent install command response',
      );
    });
  });
});
