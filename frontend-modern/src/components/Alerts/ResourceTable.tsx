...
export function ResourceTable(props: ResourceTableProps) {
  ...
  const totalColumnCount = () => props.columns.length + 3 + (props.showOfflineAlertsColumn ? 1 : 0) + (props.onBulkEdit ? 1 : 0);
 ...
 <span class="text-[10px] uppercase font-bold tracking-wider">{column.replace(/mb\/s|%|°c/gi,'').trim()}</span>
 ...
 <TableHead class="text-left w-1/4">
 Resource
 </TableHead>
 <For each={props.columns}>
 {(column) => (
