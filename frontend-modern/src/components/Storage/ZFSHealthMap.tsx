        ...
        <Portal mount={document.body}>
          ...
          >
            <div class="bg-surface text-base-content text-[10px] rounded-md shadow-sm px-2 py-1.5 min-w-[120px] border border-border">
              <div class="font-medium mb-0.5 text-base-content">{hoveredDevice()?.name}</div>
              <div class="text-muted mb-1">{hoveredDevice()?.type}</div>
              <div class="flex items-center gap-2 border-t border-border pt-1">
                <span
                  class={`font-semibold ${
                    hoveredDevice()?.state === 'ONLINE'
                      ? 'text-green-400'
                      : hoveredDevice()?.state === 'DEGRADED'
                        ? 'text-yellow-400'
                        : 'text-red-400'
                  }`}
                >
                ...
                <Show
                  when={
                    hoveredDevice()?.readErrors ||
                    hoveredDevice()?.writeErrors ||
                    hoveredDevice()?.checksumErrors
                  }
                >
                  <span class="text-red-400">
                    (E: {hoveredDevice()?.readErrors}/{hoveredDevice()?.writeErrors}/
                    {hoveredDevice()?.checksumErrors})
                  </span>
                </Show>
              </div>
