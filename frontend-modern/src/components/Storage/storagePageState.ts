...
const normalizeStorageRouteToken = (value: string): string => value.trim().toLowerCase();

const normalizeStorageRouteSelection = (value: string | undefined): string => {
  const normalized = (value || '').trim();
  return normalized.toLowerCase() === DEFAULT_STORAGE_SELECTED_NODE_ID
...
export const normalizeStorageView = (value: string): StorageView =>
  normalizeStorageRouteToken(value) === 'disks' ? 'disks' : 'pools';

export const normalizeStorageSortDirection = (value: string): 'asc' | 'desc' =>
  normalizeStorageRouteToken(value) === 'asc' ? 'asc' : 'desc';
  ...
  source: {
    ...
    read: (parsed) =>
      normalizeSourcePlatformQueryValue(
        readStorageRouteValue(parsed.source, DEFAULT_STORAGE_SOURCE_FILTER),
      ) || DEFAULT_STORAGE_SOURCE_FILTER,
    write: (value) =>
      writeStorageRouteValue(
        normalizeSourcePlatformQueryValue(value) || DEFAULT_STORAGE_SOURCE_FILTER,
        DEFAULT_STORAGE_SOURCE_FILTER,
      ),
  },
  status: {
