...
describe('useStoragePageFilters', () => {
  ...
  it('canonicalizes source aliases from the storage URL through the shared page-filter hook', () => {
    const ctx = setup('?source=%20PVE%20');
  ...
  it('collapses explicit all node sentinels from the storage URL through the shared page-filter hook', () => {
    const ctx = setup('?node=%20ALL%20');
