...
const QueueIndicator: Component<{ queue?: { total: number; active?: number; deferred?: number; hold?: number; incoming?: number; oldestAge?: number } }> = (props) => {
 ...
 <div class="bg-surface text-base-content text-xs rounded-md px-3 py-2 shadow-sm border border-border">
 <div class="space-y-1">
 ...
 <div class="flex justify-between">
 <span class="text-muted">Incoming</span>
...
export const PMGInstancePanel: Component<{ pmg: PMGInstance }> = (props) => {
 ...
 <TableRow class="hover:bg-surface-hover">
 <TableCell class="px-3 py-2 font-medium text-base-content">{row.domain}</TableCell>
 <TableCell class="px-3 py-2 text-base-content">{fmtMaybe(row.mail)}</TableCell>
 ...
 <TableCell class="px-3 py-2 text-base-content">{formatPct(row.spamRate)}</TableCell>
 <TableCell class="px-3 py-2 text-muted">{row.bytes > 0 ? formatBytes(row.bytes) : '—'}</TableCell>
 </TableRow>
 )}
 </For>
 </TableBody>
