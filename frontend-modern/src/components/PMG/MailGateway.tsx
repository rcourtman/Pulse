...
const MailGateway: Component = () => {
  ...
  const aggregateStats = createMemo(() => {
    ...
    for (const pmg of pmgs) {
      ...
      if (q) {
        totalQuarantine +=
          (q.spam || 0) + (q.virus || 0) + (q.attachment || 0) + (q.blacklisted || 0);
      }
            ...
            <Card padding="sm" tone={aggregateStats().spamRate > 50 ? 'warning' : 'card'}>
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <div
                    class={`text-2xl font-bold mt-1 ${aggregateStats().spamRate > 50 ? 'text-orange-600 dark:text-orange-400' : 'text-base-content'}`}
                  >
                ...
                <div
                  class={`p-2 rounded-md ${aggregateStats().spamRate > 50 ? 'bg-orange-100 dark:bg-orange-900' : 'bg-orange-100 dark:bg-orange-900'}`}
                >
                  <svg
            ...
            <Card padding="sm" tone={aggregateStats().totalVirus > 0 ? 'danger' : 'card'}>
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <div
                    class={`text-2xl font-bold mt-1 ${aggregateStats().totalVirus > 0 ? 'text-red-600 dark:text-red-400' : 'text-base-content'}`}
                  >
                    {formatCompact(aggregateStats().totalVirus)}
                  </div>
                  <div class="text-xs text-muted">blocked today</div>
                </div>
                <div
                  class={`p-2 rounded-md ${aggregateStats().totalVirus > 0 ? 'bg-red-100 dark:bg-red-900' : 'bg-surface-alt'}`}
                >
                  <svg
                    class={`w-5 h-5 ${aggregateStats().totalVirus > 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-400'}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="1.5"
            ...
            <Card padding="sm" tone="card">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <div class="text-xs text-muted">items held</div>
                </div>
                ...
                <div>
                  ...
                  <div
                    class={`text-2xl font-bold mt-1 ${
                      aggregateStats().totalQueue > 100
                        ? 'text-red-600 dark:text-red-400'
                        : aggregateStats().totalQueue > 20
                          ? 'text-yellow-600 dark:text-yellow-400'
                          : 'text-green-600 dark:text-green-400'
                    }`}
                  >
            ...
            <div class="space-y-4">
              <For each={filteredInstances()}>
                {(pmg) => {
                  const lastUpdated = () => {
                    const ts =
                      parseTimestamp(pmg.mailStats?.updatedAt) ??
                      parseTimestamp(pmg.lastUpdated) ??
                      parseTimestamp(pmg.lastSeen);
                        ...
                        <div class="mt-3 pt-3 border-t border-border">
                          ...
                          <div class="flex flex-wrap gap-x-6 gap-y-1 text-xs">
                            ...
                            <div class="flex items-center gap-2">
                              ...
                              <span class="font-medium text-base-content">
                                {formatNum(stats().pregreet)}
                              </span>
                            </div>
                            <div class="flex items-center gap-2">
                              <span class="text-muted">Greylisted:</span>
                              <span class="font-medium text-base-content">
                                {formatNum(stats().greylist)}
                              </span>
                            </div>
                            <div class="flex items-center gap-2">
                            ...
                            <Show when={pmg.mailStats?.averageProcessTimeMs}>
                              <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-base-content">
                                  {formatDec((pmg.mailStats?.averageProcessTimeMs || 0) / 1000, 2)}s
                                </span>
                              </div>
                            </Show>
                          </div>
                          ...
                          <div class="mt-3 pt-3 border-t border-border">
                            ...
                            <div class="flex gap-1 overflow-x-auto pb-1">
                              <For each={pmg.spamDistribution}>
                                {(bucket) => {
                                  const totalScored =
                                    pmg.spamDistribution?.reduce((sum, b) => sum + b.count, 0) || 1;
