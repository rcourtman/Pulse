  ...
  const groupedByDay = createMemo(() => {
    ...
    for (const p of sortedPoints()) {
      const key = p.completedAt
        ? recoveryDateKeyFromTimestamp(Date.parse(p.completedAt))
        : 'unknown';
      if (!groupMap.has(key)) {
          ...
          <div class="border-b border-border px-3 py-3">
            <PageControls
              search={
                <SearchInput
                  value={queryFilter}
                  onChange={(value) => setQueryFilter(value)}
              ...
              mobileFilters={{
                enabled: isMobile(),
                onToggle: () => setProtectedFiltersOpen((o) => !o),
        ...
        <Show when={!recoveryRollups.rollups.loading && recoveryRollups.rollups.error}>
          <div class="p-6">
            <EmptyState
              ...
              description={String(
                (recoveryRollups.rollups.error as Error)?.message || recoveryRollups.rollups.error,
              )}
            />
          </div>
        </Show>
                ...
                <div class="flex flex-col gap-2">
                  <div class="flex flex-wrap items-center gap-3 text-sm">
                    ...
                    <Show when={overallRollupsSummary().stale > 0}>
                      <span class="text-amber-500">{overallRollupsSummary().stale} stale</span>
                    </Show>
            ...
            <Show when={!kioskMode()}>
              <div class="border-b border-border px-3 py-3">
                <PageControls
                  ...
                  mobileFilters={{
                    enabled: isMobile(),
                    onToggle: () => setHistoryFiltersOpen((o) => !o),
                    ...
                    <div class="ml-auto flex items-center gap-2">
                      <div class="relative">
                        ...
                        <Show when={moreFiltersOpen()}>
                          <FilterToolbarPanel
                            ref={advancedFiltersPanelRef}
                            id="recovery-filter-panel"
                          >
                              ...
                              <Show when={showNamespaceFilter()}>
                                <label class="flex min-w-0 flex-col gap-1">
                                  ...
                                  >
                                    ...
                                    <For
                                      each={namespaceOptions().filter((value) => value !== 'all')}
                                    >
                                      {(namespace) => (
