...
const DiagnosticCard: Component<{
  ...
  return (
    ...
    >
      ...
      <div class="text-xs text-muted space-y-1.5">{props.children}</div>
    </div>
  );
};
...
export const DiagnosticsPanel: Component = () => {
  ...
  return (
    <div class="space-y-6">
      ...
      {/* Diagnostics Content */}
      <Show
        when={diagnosticsData()}
        fallback={
          <Card padding="lg" class="text-center">
            <div class="py-12">
              <Activity class="w-12 h-12 mx-auto text-muted mb-4" />
              <h3 class="text-lg font-medium text-base-content mb-2">No diagnostics data yet</h3>
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          ...
          <DiagnosticCard title="System Runtime" icon={Cpu} status="info">
            <MetricRow
              label="OS / Arch"
              value={`${diagnosticsData()?.system?.os || '?'} / ${diagnosticsData()?.system?.arch || '?'}`}
            />
              ...
              <For each={diagnosticsData()?.nodes || []}>
                {(node) => (
                  <div class="flex items-center justify-between py-1 border-b border-border-subtle last:border-0">
                    <span class="truncate max-w-[120px]" title={node.host}>
                      {node.name}
                    </span>
          ...
          <DiagnosticCard
            ...
            icon={HardDrive}
            status={
              diagnosticsData()?.pbs?.every((p) => p.connected)
                ? 'success'
                : diagnosticsData()?.pbs?.length
                  ? 'warning'
                  : 'info'
            }
          >
            ...
            >
              ...
              <div class="space-y-1">
                <For each={diagnosticsData()?.pbs || []}>
                  {(pbs) => (
                    <div class="flex items-center justify-between py-1 border-b border-border-subtle last:border-0">
                      <span class="truncate max-w-[120px]" title={pbs.host}>
                        {pbs.name}
                      </span>
          ...
          <Show when={diagnosticsData()?.metricsStore}>
            <Card padding="md">
              ...
              <div class="space-y-2 text-xs">
                <MetricRow
                  label="Enabled"
                  value={diagnosticsData()?.metricsStore?.enabled ? 'Yes' : 'No'}
                />
                <MetricRow
                  label="DB Size"
                  value={`${Math.round((diagnosticsData()?.metricsStore?.dbSize ?? 0) / (1024 * 1024))} MB`}
                />
                <MetricRow
                  label="Total Points"
                  value={diagnosticsData()?.metricsStore?.totalPoints ?? 0}
                />
                <MetricRow
                  label="Raw Points"
                  value={diagnosticsData()?.metricsStore?.rawCount ?? 0}
                />
                <MetricRow
                  label="Minute Points"
                  value={diagnosticsData()?.metricsStore?.minuteCount ?? 0}
                />
                <MetricRow
                  label="Hourly Points"
                  value={diagnosticsData()?.metricsStore?.hourlyCount ?? 0}
                />
                <MetricRow
                  label="Daily Points"
                  value={diagnosticsData()?.metricsStore?.dailyCount ?? 0}
                />
                <MetricRow
                  label="Buffer Size"
                  value={diagnosticsData()?.metricsStore?.bufferSize ?? 0}
                />
              </div>
            ...
            <Card padding="md">
              ...
              <div class="space-y-2 text-xs">
                <MetricRow
                  ...
                  value={diagnosticsData()?.apiTokens?.tokenCount}
                />
                <MetricRow
                  label="Unused Tokens"
          ...
          <Show when={diagnosticsData()?.alerts}>
            <Card padding="md">
              ...
              <div class="flex flex-wrap gap-2">
                ...
                >
                  Grouping:{' '}
                  {diagnosticsData()?.alerts?.missingGroupingWindow ? 'Disabled' : 'Enabled'}
                </span>
              </div>
            ...
            <Card padding="md">
              <div class="flex items-center gap-3 mb-4 pb-3 border-b border-border">
                ...
                <div class="ml-auto">
                  <StatusBadge
                    status={
                      diagnosticsData()?.aiChat?.running
                        ? 'online'
                        : diagnosticsData()?.aiChat?.enabled
                          ? 'offline'
                          : 'unknown'
                    }
                    label={
              ...
              <div class="space-y-2 text-xs">
                ...
                <MetricRow
                  label="Status"
                  value={diagnosticsData()?.aiChat?.healthy ? 'Healthy' : 'Unhealthy'}
                />
              </div>
