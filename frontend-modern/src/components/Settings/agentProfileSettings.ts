...
export const KNOWN_SETTINGS: KnownSetting[] = [
  ...
  },
  {
    ...
    description: 'Monitor Kubernetes workloads',
  },
  {
    key: 'kube_include_all_pods',
    type: 'boolean',
    label: 'Include All Pods',
    description: 'Include all non-succeeded pods in reports',
  },
  {
    key: 'kube_include_all_deployments',
    type: 'boolean',
    label: 'Include All Deployments',
    description: 'Include all deployments, not just problem ones',
  },
  {
    key: 'enable_proxmox',
    type: 'boolean',
    label: 'Enable Proxmox Mode',
    description: 'Auto-detect and configure Proxmox API access',
  },
  {
    key: 'proxmox_type',
    type: 'select',
    label: 'Proxmox Type',
    description: 'Force PVE or PBS mode',
    options: ['auto', 'pve', 'pbs'],
  },
  {
    key: 'disable_ceph',
    type: 'boolean',
    label: 'Disable Ceph Monitoring',
    description: 'Skip local Ceph status polling',
  },
  // Timing
  {
    key: 'interval',
    ...
    description: 'How often the agent reports metrics (e.g., 30s, 1m)',
  },
  // Network
  {
    key: 'report_ip',
    type: 'string',
    label: 'Report IP Override',
    description: 'Override the reported IP address',
    placeholder: '',
  },
  // Operations
  {
    ...
    description: 'Stop the unified agent from auto-updating',
  },
  {
    key: 'disable_docker_update_checks',
    type: 'boolean',
    label: 'Disable Docker Update Checks',
    description: 'Skip Docker image update detection (avoid registry rate limits)',
  },
  {
    key: 'log_level',
    type: 'select',
...
export const KNOWN_SETTINGS_BY_KEY = new Map(
  KNOWN_SETTINGS.map((setting) => [setting.key, setting]),
);
