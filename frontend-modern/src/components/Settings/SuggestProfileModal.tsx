    ...
    try {
      ...
      setSchemaError(
        err instanceof Error && err.message
          ? err.message
          : 'Defaults and validation details are unavailable right now.',
      );
    }
  });
      ...
      (async () => {
        try {
          ...
          setValidationError(
            err instanceof Error && err.message
              ? err.message
              : 'Validation unavailable right now. You can still review the draft.',
          );
          setValidation(null);
        }
      })();
      ...
      <div class="w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-lg font-semibold text-base-content">Profile Ideas</h3>
                        ...
                        <For each={validation()?.errors || []}>
                          {(issue) => (
                            <li class="flex items-start gap-2">
                              <span class="font-mono text-red-700 dark:text-red-200">
                                {issue.key}
                              </span>
                ...
                <Show when={riskHints().length > 0}>
                  <div class="flex items-start gap-2 p-3 rounded-md bg-amber-50 dark:bg-amber-900 border border-amber-200 dark:border-amber-800">
                    ...
                    <div class="space-y-1">
                      ...
                      <ul class="text-xs text-amber-700 dark:text-amber-300 space-y-1">
                        <For each={riskHints()}>{(hint) => <li>{hint}</li>}</For>
                      </ul>
                    </div>
                  </div>
                </Show>
                  ...
                  <div class="p-4 space-y-4">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      <div class="space-y-3">
                        ...
                        <Show
                          when={settingsPreview().length > 0}
                          fallback={<p class="text-xs text-muted">No settings were suggested.</p>}
                        >
                          <div class="space-y-2">
                          ...
                          <div class="rounded-md border border-border p-3">
                            <div class="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <p class="text-xs text-muted mt-1">
                                  Only overrides are shown. {omittedDefaults().length} settings stay
                                  at defaults.
                                </p>
                              </div>
                              <button
                            ...
                            <Show when={showDefaults()}>
                              <div class="mt-3 space-y-2">
                                <Show
                                  when={omittedDefaults().length > 0}
                                  fallback={<p class="text-xs text-muted">No defaults to show.</p>}
                                >
                ...
                <Show when={history().length > 1}>
                  <div class="rounded-md border border-border p-4 space-y-2">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="text-xs text-muted">{history().length - 1} older</span>
                    </div>
                    <p class="text-xs text-muted">Switch to a previous suggestion.</p>
                    <div class="space-y-2">
