...
describe('agentActionIdForResource', () => {
  ...
  // The branches below drive each still-uncovered resolution arm of
  // getActionableAgentIdFromResource (the helper whose truthiness selects the
  // first operand of agentActionIdForResource's `||`). Every case asserts the
  // concrete string that agentActionIdForResource returns, proving which arm
  // of the `||` fired and which source inside the helper won the short-circuit.

  it('resolves from the top-level resource.agent.agentId facet (first explicit-id arm)', () => {
    expect(
      agentActionIdForResource(
        makeResource({ id: 'fallback-id', agent: { agentId: 'facet-agent-1' } }),
      ),
    ).toBe('facet-agent-1');
  });

  it('resolves from platformData.agentId when no agent facet is present (third explicit-id arm)', () => {
    expect(
      agentActionIdForResource(
        makeResource({ id: 'fallback-id', platformData: { agentId: 'pd-agent-2' } }),
      ),
    ).toBe('pd-agent-2');
  });

  it('resolves from platformData.linkedAgentId when agentId is absent (fourth explicit-id arm)', () => {
    expect(
      agentActionIdForResource(
        makeResource({ id: 'fallback-id', platformData: { linkedAgentId: 'linked-3' } }),
      ),
    ).toBe('linked-3');
  });

  it('resolves from resource.kubernetes.agentId (fifth explicit-id arm)', () => {
    expect(
      agentActionIdForResource(
        makeResource({ id: 'fallback-id', kubernetes: { agentId: 'k8s-agent-4' } }),
      ),
    ).toBe('k8s-agent-4');
  });

  it('resolves from platformData.kubernetes.agentId (sixth explicit-id arm)', () => {
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
          platformData: { kubernetes: { agentId: 'pd-k8s-5' } },
        }),
      ),
    ).toBe('pd-k8s-5');
  });

  it('prefers resource.agent.agentId over platformData.agent.agentId (short-circuit priority)', () => {
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
          agent: { agentId: 'preferred' },
          platformData: { agent: { agentId: 'shadowed' } },
        }),
      ),
    ).toBe('preferred');
  });

  it('resolves from discoveryTarget.resourceId for an agent discovery target (getAgentDiscoveryResourceId resourceId arm)', () => {
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
  ...
  it('falls back to discoveryTarget.agentId inside getAgentDiscoveryResourceId when resourceId is blank', () => {
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
  ...
  it('normalizes a case-variant discovery resourceType ("AGENT") onto the agent arm', () => {
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
          discoveryTarget: {
            resourceType: 'AGENT' as unknown as ResourceDiscoveryTarget['resourceType'],
            agentId: 'dt-agent-8',
            resourceId: 'dt-resource-8',
          },
        }),
      ),
    ).toBe('dt-resource-8');
  ...
  it('skips getAgentDiscoveryResourceId for a non-agent discovery target and falls through to discoveryTarget.agentId', () => {
    // resourceType 'vm' fails isAgentDiscoveryResourceType, so
    // getAgentDiscoveryResourceId returns undefined; the final arm of
    // getActionableAgentIdFromResource reads discoveryTarget.agentId directly.
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
          discoveryTarget: {
            resourceType: 'vm',
            agentId: 'vm-host-agent-9',
            resourceId: 'vm-resource-9',
          },
        }),
      ),
    ).toBe('vm-host-agent-9');
  });

  it('treats whitespace-only explicit agent ids as missing and falls back to resource.id', () => {
    // asTrimmedString returns undefined for every whitespace-only value, so
    // every explicit-id arm is skipped and the `||` selects resource.id.
    expect(
      agentActionIdForResource(
        makeResource({
          id: 'fallback-id',
          agent: { agentId: '   ' },
          platformData: { agent: { agentId: '\t' }, linkedAgentId: ' ' },
        }),
      ),
    ).toBe('fallback-id');
  });

  it('returns resource.id when discoveryTarget is absent and no explicit agent id exists', () => {
    expect(
      agentActionIdForResource(
        makeResource({ id: 'fallback-id', type: 'storage', platformType: 'generic' }),
      ),
    ).toBe('fallback-id');
  });
});
