    ...
    <div class="space-y-4">
      <Card class="p-5">
        <PageHeader
          ...
          description={
            <>
              Pulse now uses a resource-first layout: Infrastructure, Workloads, Storage, and
              Recovery. Legacy URLs still redirect for compatibility, but this guide shows the
        ...
        <div class="mt-3 text-xs text-muted space-y-1">
          ...
          <div>
            Unified resources enable one inventory, one search, and consistent filters across
            Proxmox, agents, Docker, Kubernetes, and new sources. The goal is fewer duplicated pages
            and a navigation model that scales as integrations expand.
          </div>
        </div>
        <p class="mt-2 text-xs text-amber-700 dark:text-amber-300">
          Deprecation policy: legacy URLs exist as compatibility aliases. Update bookmarks to
          canonical routes. Tip: use the Command Palette (<span class="font-mono">Cmd+K</span>) to
          jump to the new destinations by typing what you remember.
        </p>
      </Card>
        ...
        <Table class="w-full border-collapse">
          ...
          <TableBody class="bg-surface divide-y divide-border-subtle">
            <For each={ROUTE_MAPPINGS}>
              {(item) => (
                <TableRow>
                  <TableCell class="px-4 py-2 text-sm font-mono text-base-content">
                    {item.legacy}
                  </TableCell>
                  <TableCell class="px-4 py-2 text-sm font-mono text-blue-700 dark:text-blue-300">
                    {item.destination}
                  </TableCell>
