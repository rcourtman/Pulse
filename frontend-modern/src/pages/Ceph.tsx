...
const ServiceStatusCell: Component<{ services: CephServiceStatus[] }> = (props) => {
  ...
  const getServiceStatus = (svc: CephServiceStatus) => {
    if (svc.running === svc.total)
      return { color: 'text-green-600 dark:text-green-400', status: 'healthy' };
    if (svc.running > 0)
      return { color: 'text-yellow-600 dark:text-yellow-400', status: 'degraded' };
          ...
          <Show when={allPools().length > 0}>
            <Card padding="none" tone="card" class="overflow-hidden">
              ...
              <Show
                ...
                fallback={
                  <div class="p-8 text-center text-muted">No pools match "{searchTerm()}"</div>
                }
              >
                <div
