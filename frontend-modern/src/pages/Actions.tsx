...
export function Actions() {
  ...
  return (
    <div class="mx-auto w-full max-w-6xl space-y-5 px-3 py-4 sm:px-5">
      <PageHeader
        title="Actions"
        description="Review every pending change and its recorded outcome in one place."
      />
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          <button
            ...
            class={`rounded px-4 py-2 text-sm font-medium ${view() === 'pending' ? 'bg-blue-600 text-white' : 'text-muted hover:text-base-content'}`}
          ...
          <button
            ...
            class={`rounded px-4 py-2 text-sm font-medium ${view() === 'settled' ? 'bg-blue-600 text-white' : 'text-muted hover:text-base-content'}`}
      ...
      <Show when={readOnly()}>
        <div class="rounded-lg border border-blue-300 bg-blue-50 p-3 text-sm text-blue-900 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-100">
          Mock action history is read-only. Open any record to inspect its plan, policy evidence,
      ...
      <Show when={!loading() && !loadError() && actions().length === 0}>
        <div
          ...
          class="rounded-xl border border-dashed border-border bg-surface p-8 text-center"
        >
      ...
      <Show when={actions().length > 0}>
        <ul
          class="space-y-3"
          aria-label={view() === 'pending' ? 'Actions needing attention' : 'Action history'}
        >
          <For each={actions()}>
            {(action) => (
              <li>
                <button
                  type="button"
                  class="w-full rounded-xl border border-border bg-surface p-4 text-left transition hover:border-blue-400 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500"
                  onClick={() => void openAction(action)}
                >
                  <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div class="min-w-0">
                      <div class="font-semibold">
                        {formatActionName(action.request.capabilityName)}
                      </div>
                      <div class="mt-1 break-all text-sm text-muted">
                        {action.request.resourceId}
                      </div>
                      <p class="mt-2 text-sm">{action.request.reason}</p>
                    </div>
                    <div class="shrink-0 text-left sm:text-right">
                      <span class="inline-flex rounded-full border border-border-subtle bg-surface-hover px-2.5 py-1 text-xs font-medium">
                        {formatActionName(action.state)}
                      </span>
                      <div class="mt-2 text-xs text-muted">
                        {formatRelativeTime(action.updatedAt)}
                      </div>
                    </div>
                  </div>
                </button>
