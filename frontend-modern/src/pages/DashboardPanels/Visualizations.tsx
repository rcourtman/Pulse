...
export const MiniDonut: Component<MiniDonutProps> = (props) => {
    ...
    return (
        <div class={`relative inline-flex items-center justify-center ${props.className || ''}`} style={{ width: `${size}px`, height: `${size}px` }}>

            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} class={`absolute inset-0 rotate-[-90deg]`}>
                <For each={(() => {
                    let accumulated = 0;
                    const t = total();
                    if (t === 0) return [];
