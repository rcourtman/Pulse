...
export function CompositionPanel(props: CompositionPanelProps) {
  const infraTypes = () =>
    Object.entries(props.infrastructureByType).filter(
      (item): item is [string, number] => item[1] > 0,
    );
  const workloadTypes = () =>
    Object.entries(props.workloadsByType).filter((item): item is [string, number] => item[1] > 0);
  ...
  return (
    <Card padding="none" class="px-4 py-3.5">
      <div class="flex items-center gap-2 mb-3">
        <h2 class="text-xs font-semibold text-muted uppercase tracking-wide">Composition</h2>
      </div>

      <div class="space-y-4">
        <Show when={totalInfra() > 0}>
          <div class="space-y-2">
            <h3 class="text-[10px] font-bold text-muted uppercase tracking-wider">
              Infrastructure
            </h3>
            <div class="space-y-3">
              <For each={infraTypes()}>
