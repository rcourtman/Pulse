 ...
 */
export async function setFindingNote(
  findingId: string,
  note: string,
): Promise<{ success: boolean; message: string }> {
...
export async function getInvestigation(findingId: string): Promise<Investigation> {
  return apiFetchJSON<Investigation>(
    `/api/ai/findings/${encodeURIComponent(findingId)}/investigation`,
  );
}
...
export async function getInvestigationMessages(findingId: string): Promise<InvestigationMessages> {
  return apiFetchJSON<InvestigationMessages>(
    `/api/ai/findings/${encodeURIComponent(findingId)}/investigation/messages`,
  );
}
 ...
 */
export async function reinvestigateFinding(
  findingId: string,
): Promise<{ success: boolean; message: string }> {
 ...
 * @param limit Maximum number of runs to return (default: 30)
 */
export async function getPatrolRunHistoryWithToolCalls(
  limit: number = 30,
