...
> = {
  ...
  'policy_authorization_invalid:': {
    ...
    detail:
      'Pulse refused dispatch because current server policy authority could not be validated.',
    className:
      'border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-800 dark:bg-rose-950/40 dark:text-rose-300',
  },
...
export const getActionAuditResultPresentation = (
  ...
  if (truth) {
    ...
    return {
      ...presentation,
      detail: truth.summary?.trim() || undefined,
      reasonLabel: truth.reasonCode ? formatActionCapabilityLabel(truth.reasonCode) : undefined,
    };
  }
...
export const getActionAuditVerificationOutcomePresentation = (audit: {
  ...
  if (truth) {
    const source =
      truth.evidenceClass === 'independent'
        ? 'Independent observer'
        : truth.evidenceClass === 'agent_attested'
          ? 'Executing agent (agent-attested)'
          : 'No evidence source';
    const statusCopy = {
      confirmed:
        truth.evidenceClass === 'independent'
          ? [
              ...
              'An observer in a different trust domain confirmed the intended state.',
            ]
          : truth.evidenceClass === 'agent_attested'
            ? [
                'Confirmed by executing agent',
                'The same agent trust domain that executed the action reported the intended state.',
              ]
            : [
      ...
      inconclusive: [
        ...
        'Available evidence could not establish the intended state.',
      ],
  ...
  const presentation = VERIFICATION_OUTCOME_PRESENTATION[
