...
describe('selfHostedPlans', () => {
  it('keeps the monitored-system definition explicit and child-resource inclusive', () => {
    expect(SELF_HOSTED_MONITORED_SYSTEMS_DESCRIPTION).toContain(
      'top-level machine or cluster',
    );
    expect(SELF_HOSTED_MONITORED_SYSTEMS_DESCRIPTION).toContain('counts once');
    expect(SELF_HOSTED_MONITORED_SYSTEMS_DESCRIPTION).toContain('VMs');
    expect(SELF_HOSTED_MONITORED_SYSTEMS_DESCRIPTION).toContain('services');
  });
