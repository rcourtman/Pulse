...
describe('actionAuditPresentation branch coverage (supplemental)', () => {
  ...
  describe('shouldRenderActionAuditVerification', () => {
    ...
    it('suppresses the legacy verification UI once result.actionResultV2 is present (=== undefined false arm)', () => {
      // The function's central purpose: when the v2 truth layer exists, the
      // `audit.result?.actionResultV2 === undefined` guard is false, so the `&&`
      // short-circuits to false even though a legacy verification ran. The prior
      // tests only ever set actionResultV2-absent inputs, leaving this arm open.
      expect(
        shouldRenderActionAuditVerification({
          verification: { ran: true, success: true },
          result: {
            success: true,
            actionResultV2: {
              version: 2,
              execution: { status: 'succeeded' },
              verification: { status: 'confirmed', evidenceClass: 'independent' },
    ...
    it('still suppresses when actionResultV2 is present but the legacy verification did not run', () => {
      // Same false arm of the guard, now with ran === false to confirm the
      // short-circuit dominates regardless of the verification value.
      expect(
        shouldRenderActionAuditVerification({
          result: {
            success: true,
            verification: { ran: false, success: false },
            actionResultV2: {
              version: 2,
    ...
    it('returns false when a result exists without actionResultV2 and no verification object at all', () => {
      // guard true (actionResultV2 undefined), but getActionAuditVerification(audit)
      // resolves to undefined -> `undefined?.ran === true` is false. Distinct from
      // the empty-record case above because `result` is present here.
      expect(shouldRenderActionAuditVerification({ result: { success: true } })).toBe(false);
    });

    it('lets the top-level verification precedence force a false render (ran=false shadows a ran=true result verification)', () => {
      // getActionAuditVerification = audit.verification ?? audit.result?.verification:
      // the top-level object wins, so its ran=false forces false even though the
      // result-embedded verification ran. Documents the ?? left-operand behaviour
      // in the shouldRender composition.
      expect(
        shouldRenderActionAuditVerification({
          verification: { ran: false, success: false },
          result: { success: true, verification: { ran: true, success: true } },
