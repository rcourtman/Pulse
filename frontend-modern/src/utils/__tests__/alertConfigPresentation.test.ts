...
describe('alertConfigPresentation', () => {
  it('returns canonical alert config shell vocabulary', () => {
    ...
    expect(ALERT_CONFIG_ESCALATION_DESTINATIONS_HELP).toContain('exact destinations');
    expect(ALERT_CONFIG_ESCALATION_DESTINATION_DISABLED).toBe('disabled');
    expect(ALERT_CONFIG_ESCALATION_DESTINATION_UNAVAILABLE).toBe('Unavailable destination');
    expect(ALERT_CONFIG_ESCALATION_REPEAT_LABEL).toBe(
      'Repeat critical escalation until acknowledged',
    );
    expect(ALERT_CONFIG_ESCALATION_REPEAT_EVERY_LABEL).toBe('Repeat every');
    expect(ALERT_CONFIG_ESCALATION_REPEAT_HELP).toContain(
      'acknowledged, resolved, snoozed, or notifications are paused',
    );
