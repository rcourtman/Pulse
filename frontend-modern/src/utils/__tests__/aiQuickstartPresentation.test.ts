...
describe('getAIQuickstartCreditsPresentation', () => {
  it('returns active quickstart credit presentation', () => {
    expect(getAIQuickstartCreditsPresentation(3, 10)).toEqual({
      ...
      title:
        '3 of 10 Patrol quickstart runs remaining on this activated Pulse Account install. No API key needed for initial Patrol quickstart.',
    });
  });
  ...
  it('returns exhausted quickstart presentation', () => {
    expect(getAIQuickstartCreditsPresentation(0, 10)).toEqual({
      ...
      title:
        'Patrol quickstart on this activated Pulse Account install is exhausted. Connect your API key to continue using Patrol.',
    });
  });
});
