...
const collectSearchCandidates = (resource: Resource): string[] => {
  const raw: Array<string | undefined> = [
    ...
    resource.kubernetes?.minAvailable,
    resource.kubernetes?.maxUnavailable,
    resource.kubernetes?.targetKind,
    resource.kubernetes?.targetName,
    ...
    ...(resource.kubernetes?.limitTypes ?? []),
    ...(resource.kubernetes?.metricTypes ?? []),
    ...Object.keys(resource.kubernetes?.hard ?? {}),
    ...Object.keys(resource.kubernetes?.used ?? {}),
