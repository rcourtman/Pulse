export type DashboardStatusTone = 'online' | 'offline' | 'warning' | 'critical' | 'unknown';

export function getDashboardStatusBadgeClass(tone: DashboardStatusTone): string {
  const base =
...
export function formatDashboardDelta(delta: number | null): string | null {
  if (delta === null) return null;
  const sign = delta >= 0 ? '+' : '';
  return `${sign}${delta.toFixed(1)}%`;
}

export function getDashboardDeltaColorClass(delta: number | null): string {
...
export const DASHBOARD_PRIORITY_ORDER: Record<DashboardActionPriority, number> = {
  critical: 0,
  high: 1,
  medium: 2,
  low: 3,
...
export const DASHBOARD_MAX_ACTION_ITEMS = 5;

export function getDashboardPriorityBadgeClass(priority: DashboardActionPriority): string {
  switch (priority) {
    case 'critical':
