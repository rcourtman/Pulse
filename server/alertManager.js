...
This alert was generated by Pulse monitoring system.
    ...
    async sendTestEmailWithCustomTransporter(config) {
        try {
            ...
            // Handle SSL/TLS errors with user-friendly messages
            if (error.code === 'ESOCKET' || error.message?.includes('wrong version number') || error.message?.includes('SSL routines')) {
                const portStr = String(port);
                let suggestion = '';
                
                if (portStr === '587' && secure === true) {
                    suggestion = 'Port 587 typically uses STARTTLS. Try setting "SMTP Secure" to OFF.';
                } else if (portStr === '465' && secure === false) {
                    suggestion = 'Port 465 typically uses SSL/TLS. Try setting "SMTP Secure" to ON.';
                } else if (portStr === '25' && secure === true) {
                    suggestion = 'Port 25 typically doesn\'t use encryption. Try setting "SMTP Secure" to OFF.';
                } else {
                    suggestion = 'Check your SMTP port and security settings. Common ports: 587 (STARTTLS), 465 (SSL/TLS), 25 (plain).';
                }
                
                return { 
                    success: false, 
                    error: `SSL/TLS configuration mismatch. ${suggestion}`
                };
            }
            ...
            if (error.code === 'ECONNREFUSED') {
                return { 
                    success: false, 
                    error: `Cannot connect to ${host}:${port}. Check if the SMTP server is running and accessible.`
                };
            }
            ...
            if (error.code === 'ETIMEDOUT' || error.code === 'ESOCKETTIMEDOUT') {
                return { 
                    success: false, 
                    error: `Connection timed out to ${host}:${port}. The server may be unreachable or blocking connections.`
                };
            }
...
Pulse Monitoring System`,
            ...
            if (error.code === 'ECONNREFUSED') {
                return { 
                    success: false, 
                    error: `Cannot connect to ${config.SMTP_HOST}:${config.SMTP_PORT}. Check if the SMTP server is running and accessible.`
                };
            }
            ...
            if (error.code === 'ETIMEDOUT' || error.code === 'ESOCKETTIMEDOUT') {
                return { 
                    success: false, 
                    error: `Connection timed out to ${config.SMTP_HOST}:${config.SMTP_PORT}. The server may be unreachable or blocking connections.`
                };
            }
