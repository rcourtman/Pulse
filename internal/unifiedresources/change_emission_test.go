...
func TestBuildResourceChange_ClassifiesRelationshipChangeAndIncludesEndpoints(t *testing.T) {
	...
	if change.From != "vm:1->db:1[Depends on]" || change.To != "vm:1->db:2[Depends on]" {
		t.Fatalf("From/To = %q/%q, want vm:1->db:1[Depends on]/vm:1->db:2[Depends on]", change.From, change.To)
	}
