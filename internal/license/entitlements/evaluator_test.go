...
func TestEvaluatorGetLimit(t *testing.T) {
	t.Run("exists", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 50}})
		limit, ok := e.GetLimit("max_monitored_systems")
		if !ok || limit != 50 {
...
func TestEvaluatorCheckLimit(t *testing.T) {
	t.Run("well under limit", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 100}})
		if got := e.CheckLimit("max_monitored_systems", 50); got != license.LimitAllowed {
	...
	t.Run("at soft threshold", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 100}})
		if got := e.CheckLimit("max_monitored_systems", 90); got != license.LimitSoftBlock {
	...
	t.Run("above soft below hard", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 100}})
		if got := e.CheckLimit("max_monitored_systems", 95); got != license.LimitSoftBlock {
			t.Fatalf("expected %q, got %q", license.LimitSoftBlock, got)
		}
	})
	...
	t.Run("at hard limit", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 100}})
		if got := e.CheckLimit("max_monitored_systems", 100); got != license.LimitHardBlock {
	...
	t.Run("over hard limit", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 100}})
		if got := e.CheckLimit("max_monitored_systems", 110); got != license.LimitHardBlock {
			t.Fatalf("expected %q, got %q", license.LimitHardBlock, got)
		}
	})
	...
	t.Run("no limit defined", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{}})
		if got := e.CheckLimit("max_monitored_systems", 1000); got != license.LimitAllowed {
			t.Fatalf("expected %q, got %q", license.LimitAllowed, got)
		}
	})

	t.Run("zero limit means unlimited", func(t *testing.T) {
		e := NewEvaluator(mockSource{limits: map[string]int64{"max_monitored_systems": 0}})
		if got := e.CheckLimit("max_monitored_systems", 10_000); got != license.LimitAllowed {
			t.Fatalf("expected %q, got %q", license.LimitAllowed, got)
		}
	})
}
