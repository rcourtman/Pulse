...
func readRepoFile(t *testing.T, rel string) string {
	...
	if useStagedGovernanceReads() {
		...
		data, err := cmd.Output()
		if err != nil {
			// Governance files live in internal/ and are not tracked in the public repo.
			// Skip gracefully so the test suite passes on a fresh clone.
			t.Skipf("governance file not available in this checkout: %s", rel)
		}
		return string(data)
	}
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			// Governance files live in internal/ and are not tracked in the public repo.
			// Skip gracefully so the test suite passes on a fresh clone.
			t.Skipf("governance file not present in this checkout: %s", rel)
		}
...
func statusJSON(t *testing.T) map[string]any {
	...
	content := readRepoFile(t, "docs/release-control/v6/internal/status.json")
...
func TestCanonicalDevelopmentProtocolExists(t *testing.T) {
	rel := "docs/release-control/v6/internal/CANONICAL_DEVELOPMENT_PROTOCOL.md"
	...
	assertContainsAll(t, rel, content, []string{
		...
		"## Boundary Rule",
		"AGENT_VALUES.md",
		"control_plane.py --agent-entrypoint --pretty",
		"status_lookup.py --lane <LANE_ID> --pretty",
		"work_claim.py --kind ... --id ... --summary ... --agent-id ... --write",
		...
		"replace the old claim with the new claim in the same `status.json` edit",
		"route through unified resources when that is the canonical shape",
		"These rules are retrospective.",
		"Existing lanes are not grandfathered out of the modernization rules.",
		"do not mark such a gap `planned`",
		"`target-update` coverage gaps",
		"Guardrail-only proof routing, contract wording, registry cleanup, and",
		"substantive lane movement by themselves.",
		"preserving legacy internals when only boundary compatibility is",
		"Legacy support is allowed only for explicit boundaries",
		"that is drift. Retire it or record the exception",
	})
}
...
func TestSubsystemRegistrySchemaExistsAndDeclaresOwnershipContract(t *testing.T) {
	rel := "docs/release-control/v6/internal/subsystems/registry.schema.json"
	content := readRepoFile(t, rel)
	assertContainsAll(t, rel, content, []string{
...
func TestReleaseControlPlaneFilesExist(t *testing.T) {
	valuesRel := "docs/release-control/internal/AGENT_VALUES.md"
	values := readRepoFile(t, valuesRel)
	assertContainsAll(t, valuesRel, values, []string{
		"# Pulse Agent Values",
		"## Purpose",
		"## Values",
		"## Delegation Rule",
		...
		"the canonical system, not the prompt, carry the detailed operating burden",
		"governance routing should usually be automatic and mostly invisible to the user",
		"Do that resolution quietly unless the result materially changes the plan",
		"control_plane.py --agent-entrypoint --pretty",
		"work_claim.py --kind ... --id ... --summary ... --agent-id ... --pretty",
		"Proof routing, registry cleanup, contract ratchets, and guardrail-only test",
		"Treat guardrail work as support work.",
		"owned runtime or product-surface delta in the same slice",
		"Keep legacy support at the boundary.",
		"Do not preserve or polish legacy-primary internal paths",
		"Prefer the largest coherent slice.",
		"Treat scores and evidence as signals, not targets.",
		"gap first, then record the proof",
		"Use worktrees when a mutating slice needs physical isolation.",
	})

	docRel := "docs/release-control/internal/CONTROL_PLANE.md"
	...
	assertContainsAll(t, docRel, doc, []string{
		...
		"Governance routing should be quiet by default",
		"status_lookup.py",
		"worktree_base.py",
		"worktree_claim.py",
		"worktree_finish.py",
...
func TestV6ControlDocsReferenceCanonicalDevelopmentProtocol(t *testing.T) {
	...
	source := readRepoFile(t, "docs/release-control/v6/internal/SOURCE_OF_TRUTH.md")
	assertContainsAll(t, "docs/release-control/v6/internal/SOURCE_OF_TRUTH.md", source, []string{
		"AGENT_VALUES.md",
		"control_plane.py --agent-entrypoint --pretty",
		"work_claim.py",
		"CONTROL_PLANE.md",
		...
		"status.json.work_claims",
		"lane-coverage score",
		"coverage-planning score",
		"active claims",
		"available_candidate_lane_queue",
		"The claim is an audit trail",
		"Lane work must be evaluated against the canonical v6 model",
		"These modernization rules are retrospective.",
		"Do not treat a lane as healthy just because it has local fixes on a legacy path.",
		"consistency, seamlessness, drift, bypass",
		"status_audit.py --pretty",
		"status_lookup.py",
		"canonical databases, not startup prompt payload",
		"Claimed lane work should prefer the largest coherent same-surface slice.",
		"Lane scores, missing evidence items, and proof counts are signals, not the",
		"Do not frame a slice as \"get two more evidence items\"",
		"Worktrees are available for isolated mutating slices",
		"Do not run parallel mutating agents out of one dirty checkout",
		"status.json.readiness_assertions",
		"`coverage_gaps.status` is not cosmetic",
		"`target-update` coverage gaps",
		"Keep governance routing quiet by default",
	})
...
func TestSourceOfTruthStaysStableAndNonOperational(t *testing.T) {
	rel := "docs/release-control/v6/internal/SOURCE_OF_TRUTH.md"
	content := readRepoFile(t, rel)
	assertContainsAll(t, rel, content, []string{
		"## Purpose",
		...
		"coverage-gap discovery records",
		"candidate-lane planning records",
		"Active multi-agent lease records",
		"normalized follow-up tracking",
		"derived coverage scores",
		...
		"These modernization rules are retrospective.",
		"Do not treat a lane as healthy just because it has local fixes on a legacy path.",
		"Legacy compatibility must be named as a boundary exception",
		"Preserve legacy behavior only at explicit boundaries",
		"Within an active claimed lane, prefer the largest coherent same-surface slice",
		"Lane scores, missing evidence items, and proof counts are signals, not the",
		"Do not frame a slice as \"get two more evidence items\"",
		"Do not treat missing evidence count or a score delta as the task itself.",
		"Worktrees are available for isolated mutating slices",
		"Do not run parallel mutating agents out of one dirty checkout",
	})
...
func TestStatusJSONHasStrictTopLevelSchema(t *testing.T) {
	...
	requiredStringFields := map[string]string{
		"version":              "6.0",
		"updated_at":           sourceOfTruthLastUpdated(t, readRepoFile(t, "docs/release-control/v6/internal/SOURCE_OF_TRUTH.md")),
	...
	if _, ok := status["coverage_gaps"].([]any); !ok {
		t.Fatalf("status.json missing coverage_gaps list")
	}
	if _, ok := status["candidate_lanes"].([]any); !ok {
		t.Fatalf("status.json missing candidate_lanes list")
	}
	if _, ok := status["work_claims"].([]any); !ok {
		t.Fatalf("status.json missing work_claims list")
	}
	if candidateLanes, ok := status["candidate_lanes"].([]any); ok {
		for index, rawCandidate := range candidateLanes {
			candidate, ok := rawCandidate.(map[string]any)
			if !ok {
				t.Fatalf("status.json candidate_lanes[%d] contains non-object entry", index)
			}
			targetID, ok := candidate["target_id"].(string)
			if !ok || strings.TrimSpace(targetID) == "" {
				t.Fatalf("status.json candidate_lanes[%d] missing target_id", index)
			}
		}
	}
...
func TestStatusJSONLaneEvidenceReferencesAreStructured(t *testing.T) {
	...
	for _, rawLane := range lanes {
		...
		if statusValue == "partial" && currentScore == 0 {
			t.Fatalf("status.json lane %q must keep current_score above 0 while status is partial", laneID)
		}
...
func TestStatusJSONCoverageGapPlanningStaysTyped(t *testing.T) {
	status := statusJSON(t)

	rawCoverageGaps, ok := status["coverage_gaps"].([]any)
	if !ok {
		t.Fatalf("status.json missing coverage_gaps list")
	}
	rawCandidateLanes, ok := status["candidate_lanes"].([]any)
	if !ok {
		t.Fatalf("status.json missing candidate_lanes list")
	}

	candidateGapRefCounts := make(map[string]int)
	for index, rawCandidate := range rawCandidateLanes {
		candidate, ok := rawCandidate.(map[string]any)
		if !ok {
			t.Fatalf("status.json candidate_lanes[%d] contains non-object entry", index)
		}
		rawGapIDs, ok := candidate["coverage_gap_ids"].([]any)
		if !ok || len(rawGapIDs) == 0 {
			t.Fatalf("status.json candidate_lanes[%d] missing coverage_gap_ids", index)
		}
		for _, rawGapID := range rawGapIDs {
			gapID, ok := rawGapID.(string)
			if !ok || strings.TrimSpace(gapID) == "" {
				t.Fatalf("status.json candidate_lanes[%d] has invalid coverage_gap_ids entry %#v", index, rawGapID)
			}
			candidateGapRefCounts[gapID]++
		}
	}

	laneShapingResolutions := map[string]struct{}{
		"new-lane":       {},
		"lane-split":     {},
		"lane-expansion": {},
	}
	for index, rawGap := range rawCoverageGaps {
		gap, ok := rawGap.(map[string]any)
		if !ok {
			t.Fatalf("status.json coverage_gaps[%d] contains non-object entry", index)
		}
		gapID, ok := gap["id"].(string)
		if !ok || strings.TrimSpace(gapID) == "" {
			t.Fatalf("status.json coverage_gaps[%d] missing id", index)
		}
		statusValue, ok := gap["status"].(string)
		if !ok || !slices.Contains([]string{"discovered", "triaged", "planned"}, statusValue) {
			t.Fatalf("status.json coverage_gap %q has invalid status %#v", gapID, gap["status"])
		}
		proposedResolution, ok := gap["proposed_resolution"].(string)
		if !ok || strings.TrimSpace(proposedResolution) == "" {
			t.Fatalf("status.json coverage_gap %q missing proposed_resolution", gapID)
		}

		_, laneShaping := laneShapingResolutions[proposedResolution]
		referencedByCandidateLane := candidateGapRefCounts[gapID] > 0
		if laneShaping && referencedByCandidateLane && statusValue != "planned" {
			t.Fatalf(
				"status.json coverage_gap %q is referenced by candidate_lanes but status is %q; lane-shaping planned gaps must be marked planned",
				gapID,
				statusValue,
			)
		}
		if laneShaping && statusValue == "planned" && !referencedByCandidateLane {
			t.Fatalf(
				"status.json coverage_gap %q is planned for %q but has no candidate_lane reference",
				gapID,
				proposedResolution,
			)
		}
		if proposedResolution == "target-update" && referencedByCandidateLane {
			t.Fatalf(
				"status.json coverage_gap %q uses target-update but is referenced by candidate_lanes",
				gapID,
			)
		}
	}
...
func TestCanonicalCompletionGuardIsWiredIntoPreCommit(t *testing.T) {
	...
	assertContainsAll(t, ".husky/pre-commit", hook, []string{
		...
		"browser_verification_guard.py",
		"Running browser verification guard...",
		...
		"browser_verification_guard_test.py",
...
func TestLegacyReleaseControlOrchestratorIsRemoved(t *testing.T) {
	...
	source := readRepoFile(t, "docs/release-control/v6/internal/SOURCE_OF_TRUTH.md")
	assertContainsNone(t, "docs/release-control/v6/internal/SOURCE_OF_TRUTH.md", source, []string{
