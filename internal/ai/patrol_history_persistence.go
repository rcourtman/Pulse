...
func (a *PatrolHistoryPersistenceAdapter) SavePatrolRunHistory(runs []PatrolRunRecord) error {
	...
	for _, r := range runs {
		...
		records = append(records, config.PatrolRunRecord{
			...
			ObjectiveContext:          clonePatrolObjectiveContext(normalized.ObjectiveContext),
...
func (a *PatrolHistoryPersistenceAdapter) LoadPatrolRunHistory() ([]PatrolRunRecord, error) {
	...
	for _, r := range data.Runs[:runCount] {
		run := normalizePatrolRunRecord(PatrolRunRecord{
			...
			ObjectiveContext:          clonePatrolObjectiveContext(r.ObjectiveContext),
			AlertIdentifier:           r.AlertIdentifier,
