...
// patrolRuntimeResolveReasonPreflight is the resolve reason a successful
// provider preflight reports; see patrol_preflight.go.
const patrolRuntimeResolveReasonPreflight = "patrol_preflight_success"
...
func (p *PatrolService) resolvePatrolRuntimeFailureFinding(reason string) bool {
	...
	existing := p.findings.Get(errorFindingID)
	if existing == nil || existing.IsResolved() {
		return false
	}
	// A healthy provider preflight says nothing about spend: a budget pause
	// stays until a run actually gets past the budget check (#1789).
	if reason == patrolRuntimeResolveReasonPreflight && existing.FailureCause == string(PatrolFailureCauseBudgetExhausted) {
		log.Debug().Str("reason", reason).Msg("AI Patrol: Keeping budget-exhausted runtime finding despite provider preflight success")
		return false
	}
