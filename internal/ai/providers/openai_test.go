...
func TestContainsControlTokens(t *testing.T) {
	tests := []struct {
		name     string
		content  string
		expected bool
		...
		{
			name:     "channel control token",
			content:  "Sure, I'll help. <|channel|> tool_call",
			expected: true,
		},
		{
			name:     "constrain control token",
			content:  "<|constrain|>json",
			expected: true,
		},
		{
			name:     "message control token",
		...
		{
			name:     "llama eot token",
			content:  "Response complete<|eot_id|>",
			expected: true,
		},
		{
			name:     "llama header tokens",
			content:  "<|start_header_id|>assistant<|end_header_id|>",
			expected: true,
		},
		{
			name:     "tool_call xml marker",
			content:  "<tool_call>get_weather</tool_call>",
			expected: true,
		},
		{
			name:     "TOOL_CALLS marker",
			content:  "[TOOL_CALLS] get_weather",
			expected: true,
		},
		{
			name:     "code with angle brackets is fine",
			content:  "Here's some code: if (a < b && c > d) { return true; }",
			expected: false,
		},
		{
			name:     "html-like content is fine",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := containsControlTokens(tt.content)
			assert.Equal(t, tt.expected, result, "containsControlTokens(%q)", tt.content)
		})
	}
}

func TestOpenAIClient_ChatStream_ControlTokenDetection(t *testing.T) {
	// Mock a server that returns control tokens (simulating an incompatible model)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		w.WriteHeader(http.StatusOK)

		// Simulate a model that outputs control tokens instead of proper tool calls
		events := []string{
			`{"id":"chatcmpl-1","choices":[{"delta":{"content":"I'll help you. "}}]}`,
			`{"id":"chatcmpl-1","choices":[{"delta":{"content":"<|channel|>"}}]}`,
			`{"id":"chatcmpl-1","choices":[{"delta":{"content":" tool_use"}}]}`,
			`[DONE]`,
		}
	...
	client := NewOpenAIClient("sk-test", "local-model", server.URL, 0)

	callback := func(event StreamEvent) {
		// We shouldn't get many events before the error
	}
	...
	err := client.ChatStream(context.Background(), req, callback)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "model_incompatible")
	assert.Contains(t, err.Error(), "function calling")
}
