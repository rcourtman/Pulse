...
func TestAIProviderClassVocabularyStaysFreeOfRetiredHostedWording(t *testing.T) {
	// The vocabulary is disclosed verbatim in the shipped privacy document,
	// which the frontend copy contract keeps free of the retired hosted
	// quickstart wording. A bucket name that reintroduces it would fail
	// that contract on the public docs, not here, so pin it at the source.
	for _, value := range AIProviderClassValues() {
		if strings.Contains(strings.ToLower(value), "quickstart") {
			t.Fatalf("provider class value %q reintroduces retired hosted wording", value)
		}
	}
	if got := ClassifyAIProviderClass(&config.AIConfig{Enabled: true, Model: config.DefaultAIModelQuickstart}); got != AIProviderClassHostedLegacy {
		t.Fatalf("legacy hosted route class = %q, want %q", got, AIProviderClassHostedLegacy)
	}
}
