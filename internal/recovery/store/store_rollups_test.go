...
func TestStore_ListRollups(t *testing.T) {
	...
	if err := store.UpsertPoints(context.Background(), []recovery.RecoveryPoint{
		...
		},
		{
			...
			Details: map[string]any{
				"k8sClusterName": "other-cluster",
				"node":           "worker-2",
			},
		},
	}); err != nil {
		t.Fatalf("UpsertPoints(extra) error = %v", err)
	}

	got3, total3, err := store.ListRollups(context.Background(), recovery.ListPointsOptions{
		Page:           1,
		Limit:          50,
	...
	if err != nil {
		t.Fatalf("ListRollups(normalized filters) error = %v", err)
	}
	if total3 != 1 || len(got3) != 1 {
		t.Fatalf("ListRollups(normalized filters) total=%d len=%d, want 1/1", total3, len(got3))
	}
