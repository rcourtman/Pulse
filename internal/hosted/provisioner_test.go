...
func TestProvisionHostedSignupIdempotentDuplicateEmailCaseInsensitive(t *testing.T) {
	baseDir := t.TempDir()
	persistence := config.NewMultiTenantPersistence(baseDir)
	authManager := &mockAuthManager{}
	authProvider := &mockAuthProvider{manager: authManager}
	...
	if authManager.calls != 0 {
		t.Fatalf("expected UpdateUserRoles to not be called for idempotent hosted signup, got %d", authManager.calls)
	}
}
