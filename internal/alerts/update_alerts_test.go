...
func TestUpdateAlertProLicenseGating(t *testing.T) {
	hostID := "docker-host-1"
	containerName := "test-container"
	instanceName := "docker-instance"
	nodeName := "docker-node"
	...
	t.Run("no license checker - alerts work", func(t *testing.T) {
		m := NewManager()
		m.mu.Lock()
		m.config.DockerDefaults.UpdateAlertDelayHours = 24
		m.mu.Unlock()

		testResourceID := "docker:" + hostID + "/container-license-test-1"
		...
		m.checkDockerContainerImageUpdate(host, container, testResourceID, containerName, instanceName, nodeName)
		...
		if !found {
			t.Error("Expected alert when no license checker is set")
		}
	})

	t.Run("pro license grants feature - alerts work", func(t *testing.T) {
		m := NewManager()
		m.mu.Lock()
		m.config.DockerDefaults.UpdateAlertDelayHours = 24
		m.mu.Unlock()
		m.SetLicenseChecker(func(feature string) bool {
			return feature == "update_alerts" // Grant the feature
		})

		testResourceID := "docker:" + hostID + "/container-license-test-2"
		...
		if !found {
			t.Error("Expected alert when Pro license grants update_alerts feature")
		}
	})

	t.Run("free license - no alerts", func(t *testing.T) {
		m := NewManager()
		m.mu.Lock()
		m.config.DockerDefaults.UpdateAlertDelayHours = 24
		m.mu.Unlock()
		m.SetLicenseChecker(func(feature string) bool {
			return false // Free tier - no features
		})

		testResourceID := "docker:" + hostID + "/container-license-test-3"
		...
		for _, a := range alerts {
			if a.Type == "docker-container-update" {
				t.Error("Expected no alert when license doesn't grant update_alerts feature")
				break
			}
		}

		// Verify tracking is also cleared
		m.mu.RLock()
		_, tracked := m.dockerUpdateFirstSeen[testResourceID]
		m.mu.RUnlock()
		if tracked {
			t.Error("Expected tracking to be cleared when license doesn't grant feature")
		}
	})
}
