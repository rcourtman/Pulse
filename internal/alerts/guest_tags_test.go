...
func TestGuestPoweredOffAlertIncludesTagsForNotificationRouting(t *testing.T) {
	...
	m.checkGuestPoweredOffWithThresholdsAndIntentAndTags(
		"qemu/route-offline",
		"customer-alpha-offline",
		"pve-prod",
		"pve-prod",
		"VM",
		m.config.GuestDefaults,
		false,
		BackupIntentContext{},
		tags,
	)
	...
	m.mu.RLock()
	alert := testRequireActiveAlert(t, m, "guest-powered-off-qemu/route-offline")
	m.mu.RUnlock()
	got, ok := alert.Metadata["tags"].([]string)
	if !ok || len(got) != 2 || got[0] != "customer:alpha" || got[1] != "critical" {
		t.Fatalf("powered-off routing tags = %#v", alert.Metadata["tags"])
	}
}
