...
kern.osrelease: 14.0-RELEASE-p6
...
hw.acpi.thermal.tz1.temperature: 42.5C
hw.machine: amd64
`
	...
	if len(got.TemperatureCelsius) != len(want) {
		t.Fatalf("got %d temperatures, want %d: %v", len(got.TemperatureCelsius), len(want), got.TemperatureCelsius)
	}
	for k, v := range want {
		if gotVal, ok := got.TemperatureCelsius[k]; !ok || gotVal != v {
			t.Fatalf("TemperatureCelsius[%q] = %v, want %v", k, gotVal, v)
		}
	}
}
...
func TestParseFreeBSDSysctlTemperatures_Empty(t *testing.T) {
	...
	if len(got.TemperatureCelsius) != 0 {
		t.Fatalf("expected empty result for no temperature data, got %v", got.TemperatureCelsius)
	}
}
