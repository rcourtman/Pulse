...
func TestOnboardingResponsesUseCanonicalEmptyCollections(t *testing.T) {
	qrJSON, err := json.Marshal(emptyOnboardingQRResponse())
	if err != nil {
		t.Fatalf("marshal empty onboarding QR response: %v", err)
	}
	if !strings.Contains(string(qrJSON), `"diagnostics":[]`) {
		t.Fatalf("expected empty onboarding QR response to retain diagnostics array, got %s", qrJSON)
	}

	deepLinkJSON, err := json.Marshal(emptyOnboardingDeepLinkResponse())
	if err != nil {
		t.Fatalf("marshal empty onboarding deep-link response: %v", err)
	}
	if !strings.Contains(string(deepLinkJSON), `"diagnostics":[]`) {
		t.Fatalf("expected empty onboarding deep-link response to retain diagnostics array, got %s", deepLinkJSON)
	}

	validationJSON, err := json.Marshal(emptyOnboardingValidationResponse())
	if err != nil {
		t.Fatalf("marshal empty onboarding validation response: %v", err)
	}
	if !strings.Contains(string(validationJSON), `"diagnostics":[]`) {
		t.Fatalf("expected empty onboarding validation response to retain diagnostics array, got %s", validationJSON)
	}
}
