...
func EmptySSOProviderResponse() SSOProviderResponse {
	return SSOProviderResponse{}.NormalizeCollections()
}

func (r SSOProviderResponse) NormalizeCollections() SSOProviderResponse {
	if r.AllowedGroups == nil {
		r.AllowedGroups = []string{}
	}
	if r.AllowedDomains == nil {
		r.AllowedDomains = []string{}
	}
...
func EmptySSOProvidersListResponse() SSOProvidersListResponse {
	return SSOProvidersListResponse{}.NormalizeCollections()
}

func (r SSOProvidersListResponse) NormalizeCollections() SSOProvidersListResponse {
	if r.Providers == nil {
		r.Providers = []SSOProviderResponse{}
	}
	for i := range r.Providers {
		r.Providers[i] = r.Providers[i].NormalizeCollections()
	}
	return r
}
...
func (r *Router) handleListSSOProviders(w http.ResponseWriter, req *http.Request) {
	...
	response := EmptySSOProvidersListResponse()
	response.DefaultProviderID = r.ssoConfig.DefaultProviderID
...
func providerToResponse(p *config.SSOProvider, publicURL string) SSOProviderResponse {
	...
	resp.ID = p.ID
	resp.Name = p.Name
	resp.Type = string(p.Type)
	resp.Enabled = p.Enabled
	resp.DisplayName = p.DisplayName
	resp.IconURL = p.IconURL
	resp.Priority = p.Priority
...
func EmptySSOTestResponse() SSOTestResponse {
	return SSOTestResponse{}.NormalizeCollections()
}

func (r SSOTestResponse) NormalizeCollections() SSOTestResponse {
	if r.Details != nil {
		normalized := r.Details.NormalizeCollections()
		r.Details = &normalized
	}
	return r
}
...
func (d SSOTestDetails) NormalizeCollections() SSOTestDetails {
	if d.Certificates == nil {
		d.Certificates = []CertificateInfo{}
	}
	if d.SupportedScopes == nil {
		d.SupportedScopes = []string{}
	}
	return d
}
...
func EmptyMetadataPreviewResponse() MetadataPreviewResponse {
	return MetadataPreviewResponse{}.NormalizeCollections()
}

func (r MetadataPreviewResponse) NormalizeCollections() MetadataPreviewResponse {
	if r.Parsed != nil {
		normalized := r.Parsed.NormalizeCollections()
		r.Parsed = &normalized
	}
	return r
}
...
type ParsedMetadataInfo struct {
	...
	SLOURL        string            `json:"sloUrl,omitempty"`
	Certificates  []CertificateInfo `json:"certificates"`
	NameIDFormats []string          `json:"nameIdFormats"`
}

func (p ParsedMetadataInfo) NormalizeCollections() ParsedMetadataInfo {
	if p.Certificates == nil {
		p.Certificates = []CertificateInfo{}
	}
	if p.NameIDFormats == nil {
		p.NameIDFormats = []string{}
	}
	return p
}
...
func (r *Router) handleMetadataPreview(w http.ResponseWriter, req *http.Request) {
	...
	response := EmptyMetadataPreviewResponse()
