...
func TestHandleExportConfig(t *testing.T) {
	...
	if err := handler.defaultPersistence.SaveNodesConfig(cfg.PVEInstances, cfg.PBSInstances, cfg.PMGInstances); err != nil {
		t.Fatalf("Failed to save initial config: %v", err)
	}
	// Also save empty settings to avoid nil pointer issues during export
	if err := handler.defaultPersistence.SaveSystemSettings(*config.DefaultSystemSettings()); err != nil {
...
func TestHandleImportConfig(t *testing.T) {
	...
	handler.defaultPersistence = dummyPersistence // Override handler's persistence
