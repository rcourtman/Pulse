...
func TestReportingRuntimeSnapshotNormalizesEmptyCollections(t *testing.T) {
	state := models.NewState()
	monitor := newReportingMonitorForTest(t, state, nil)
	...
	if snapshot.VMs == nil {
		t.Fatal("expected VMs slice to be normalized")
	}
	if snapshot.Containers == nil {
		t.Fatal("expected containers slice to be normalized")
	}
	if snapshot.ActiveAlerts == nil {
		t.Fatal("expected active alerts slice to be normalized")
	}
	if snapshot.ResolvedAlerts == nil {
		t.Fatal("expected resolved alerts slice to be normalized")
	}
	if snapshot.Storage == nil {
		t.Fatal("expected storage slice to be normalized")
	}
	if snapshot.Disks == nil {
		t.Fatal("expected disks slice to be normalized")
	}
	if snapshot.LegacyBackups == nil {
		t.Fatal("expected legacy backups slice to be normalized")
	}
}

func TestReportingEnrichmentSnapshotNormalizesEmptyCollections(t *testing.T) {
	state := models.NewState()
	monitor := newReportingMonitorForTest(t, state, nil)

	handlers := NewReportingHandlers(newReportingMTMForTest(t, monitor), nil)
	snapshot, ok := handlers.getReportingEnrichmentSnapshot(context.Background(), "default")
	if !ok {
		t.Fatal("expected enrichment snapshot to be available")
	}
	if snapshot.Nodes == nil {
		t.Fatal("expected nodes slice to be normalized")
	}
	...
	if snapshot.LegacyBackups.StorageBackups == nil {
		t.Fatal("expected legacy backup storage snapshots to be normalized")
	}
	if snapshot.Resources == nil {
		t.Fatal("expected resources slice to be normalized")
	}
}
