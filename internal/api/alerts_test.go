...
// (logged but reported as "saved successfully"), so a user setting a custom
// override would see the value stick in memory and revert on reload with
// no clue why. Surface the error so the frontend can show a real
// save-failed signal.
