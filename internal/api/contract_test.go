...
func TestContractPatrolInternalBridgePreservesBoundedToolAuthority(t *testing.T) {
	...
	if got.Prompt != "evaluate" || got.SystemPrompt != "bounded" || got.SessionID != "patrol-main" || got.ExecutionID != "run-1" || got.UseCase != "patrol" || got.MaxTurns != 5 {
...
func TestContract_AISettingsUpdateProviderResolutionJSONSnapshot(t *testing.T) {
	...
	want := fmt.Sprintf(`{
		...
		"patrol_finding_notify_min_severity":"warning",
...
// Sibling of the infrastructureRead contract above, for the admin-only System
// tabs. The two share a derivation today and are still pinned separately: the
// point of the second field is that tightening one page's gate must not
// silently move the other page's tabs, and only a test per field can catch
// that.
func TestContract_SecurityStatusSystemSettingsReadTracksSettingsReadScope(t *testing.T) {
	cases := []struct {
		name   string
		scopes []string
		want   bool
	}{
		{"settings read grants the system settings tabs", []string{config.ScopeSettingsRead}, true},
		{"monitoring read alone withholds them", []string{config.ScopeMonitoringRead}, false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rawToken := fmt.Sprintf("contract-syssettings-scope-%s.12345678", strings.ReplaceAll(tc.name, " ", "-"))
			record := newTokenRecord(t, rawToken, tc.scopes, nil)
			cfg := newTestConfigWithTokens(t, record)
			router := NewRouter(cfg, nil, nil, nil, nil, "1.0.0")
			...
			if payload.SettingsCapabilities.SystemSettingsRead != tc.want {
				t.Fatalf("settingsCapabilities.systemSettingsRead = %v, want %v",
...
func TestQuickSecuritySetupForcePreservesSystemSettings(t *testing.T) {
	...
	dir := t.TempDir()
	InitPersistentAuthStores(dir)
	hash, err := authpkg.HashPassword("ExistingPassword!1")
	if err != nil {
		t.Fatal(err)
	}
