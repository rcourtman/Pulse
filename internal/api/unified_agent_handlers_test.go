...
func newUnifiedAgentHandlers(t *testing.T, cfg *config.Config) (*UnifiedAgentHandlers, *monitoring.Monitor) {
	t.Helper()
	...
	handler := NewUnifiedAgentHandlers(nil, monitor, hub)
...
func seedUnifiedAgentHost(t *testing.T, monitor *monitoring.Monitor) string {
	t.Helper()
...
func TestUnifiedAgentHandlers_HandleReport(t *testing.T) {
...
func TestUnifiedAgentHandlers_HandleReport_EnforcesMaxMonitoredSystemsForNewHostsOnly(t *testing.T) {
	...
	existingHostID := seedUnifiedAgentHost(t, monitor)
	if existingHostID == "" {
...
func TestUnifiedAgentHandlers_HandleAllowReenroll(t *testing.T) {
	for _, prefix := range []string{"/api/agents/agent/", "/api/agents/host/"} {
		handler, monitor := newUnifiedAgentHandlers(t, nil)
		hostID := seedUnifiedAgentHost(t, monitor)
		...
		req := httptest.NewRequest(http.MethodPost, prefix+hostID+"/allow-reenroll", nil)
		rec := httptest.NewRecorder()
...
func TestUnifiedAgentHandlers_HandleConfigPatch(t *testing.T) {
	for _, prefix := range []string{"/api/agents/agent/", "/api/agents/host/"} {
		handler, monitor := newUnifiedAgentHandlers(t, nil)
		hostID := seedUnifiedAgentHost(t, monitor)
...
func TestUnifiedAgentHandlers_EnsureAgentTokenMatch(t *testing.T) {
	handler, monitor := newUnifiedAgentHandlers(t, nil)
...
func TestUnifiedAgentHandlers_HandleUninstall(t *testing.T) {
	handler, monitor := newUnifiedAgentHandlers(t, nil)
	hostID := seedUnifiedAgentHost(t, monitor)
...
func TestUnifiedAgentHandlers_HandleUninstallRejectsTokenMismatch(t *testing.T) {
	handler, monitor := newUnifiedAgentHandlers(t, nil)
	hostID := seedUnifiedAgentHost(t, monitor)
...
func TestUnifiedAgentHandlers_HandleUninstall_ResponseBody(t *testing.T) {
	handler, monitor := newUnifiedAgentHandlers(t, nil)
	hostID := seedUnifiedAgentHost(t, monitor)
...
func TestUnifiedAgentHandlers_HandleUninstall_FullLifecycle(t *testing.T) {
	handler, monitor := newUnifiedAgentHandlers(t, nil)
	state := monitorState(t, monitor)
...
func TestUnifiedAgentHandlers_HandleLinkUnlink(t *testing.T) {
	handler, monitor := newUnifiedAgentHandlers(t, nil)
	hostID := seedUnifiedAgentHost(t, monitor)
