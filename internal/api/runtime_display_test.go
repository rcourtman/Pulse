...
// The whole point of a separate type is that it cannot leak the rest of
// config.SystemSettings. Assert on the serialized object, not the Go struct: a
// future embedded struct would compile and silently publish operator
// configuration to every authenticated viewer.
...
// HandleGetSystemSettings overrides the persisted value with the effective
// runtime one so PULSE_DISABLE_DOCKER_UPDATE_ACTIONS wins. The session-tier
// projection feeds the same Docker update buttons and has to agree, or a viewer
// gets buttons the env var turned off.
