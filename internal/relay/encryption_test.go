...
func TestDeriveChannelKeys_DecryptsMobileAppVector(t *testing.T) {
	decodeHex := func(label, value string) []byte {
		t.Helper()
	...
	// Generated by repos/pulse-mobile/src/relay/encryption.ts using Node's
	// x25519 + HKDF path. This catches cross-runtime drift that Go-only and
	// TypeScript-only round trips cannot see.
	appPublic := decodeHex("app public key", "16f4f6574e7084c8f08edac52f124517131eb584bb7cd702b04c9a77c740e456")
	instancePrivate := decodeHex("instance private key", "e0fb8da5158b2076eb81c323708b9389516293c1739ea2a437bf144251df7173")
	ciphertext := decodeHex("ciphertext", "0000000000000000000000004e09ba88aed1fe11117dc092027a1d3981ec25ab3abe758d04c76a7fa4486668ca3a444a3bb9d7387c6444a3e179d8c9283911551ab164d8b80aec607c2668396d57d41867f8b6058b9b3af5e5")
	wantPlaintext := []byte(`{"id":"req_vector","method":"GET","path":"/api/ai/approvals"}`)

	instancePriv, err := ecdh.X25519().NewPrivateKey(instancePrivate)
	if err != nil {
		t.Fatalf("parse instance private key: %v", err)
	}
	appPub, err := ecdh.X25519().NewPublicKey(appPublic)
	if err != nil {
		t.Fatalf("parse app public key: %v", err)
	}

	instanceEnc, err := DeriveChannelKeys(instancePriv, appPub, true)
	if err != nil {
		t.Fatalf("derive instance channel keys: %v", err)
	}

	got, err := instanceEnc.Decrypt(ciphertext)
	if err != nil {
		t.Fatalf("decrypt mobile vector: %v", err)
	}
	if !bytes.Equal(got, wantPlaintext) {
		t.Fatalf("mobile vector plaintext = %q, want %q", got, wantPlaintext)
	}
}
