...
func TestEnsureCreatesFileAndCaches(t *testing.T) {
	...
	mgr, err := NewKnownHostsManager(path, WithKeyscanFunc(keyscan))
	if err != nil {
...
func TestEnsureReturnsErrorWhenNoEntries(t *testing.T) {
	...
	mgr, err := NewKnownHostsManager(path, WithKeyscanFunc(func(ctx context.Context, host string, port int, timeout time.Duration) ([]byte, error) {
...
func TestEnsureRespectsContextCancellation(t *testing.T) {
	...
	mgr, err := NewKnownHostsManager(path, WithKeyscanFunc(keyscan), WithTimeout(50*time.Millisecond))
	if err != nil {
		t.Fatalf("NewKnownHostsManager: %v", err)
	}
...
func TestNewManagerEmptyPath(t *testing.T) {
	if _, err := NewKnownHostsManager(""); err == nil {
...
func TestEnsureWithEntriesEnsureKnownHostsFileError(t *testing.T) {
	...
	mgr, err := NewKnownHostsManager(filepath.Join(t.TempDir(), "known_hosts"))
	if err != nil {
		t.Fatalf("NewKnownHostsManager: %v", err)
	}
...
func TestManagerPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mgr, err := NewKnownHostsManager(tt.path)
			if err != nil {
				t.Fatalf("NewKnownHostsManager(%q): %v", tt.path, err)
			}
...
func TestEnsureKnownHostsFileMkdirError(t *testing.T) {
	...
	m := &knownHostsManager{path: filepath.Join(t.TempDir(), "known_hosts")}
