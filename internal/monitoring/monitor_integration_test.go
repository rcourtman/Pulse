...
func TestMergeHostAgentSMARTIntoDisks_AgentWearoutDoesNotHideLowPVELife(t *testing.T) {
	// A worn drive whose NVMe endurance log intermittently reads
	// PercentageUsed 0 must not have its Proxmox-reported remaining life
	// raised to 100. That resolved the low-life alert and let it re-fire on the
	// next poll, producing repeated notifications (#2112).
	disks := []models.PhysicalDisk{{
		ID:      "d1",
		Node:    "pve1",
		Serial:  "SER1",
