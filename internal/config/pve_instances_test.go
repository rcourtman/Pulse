...
func TestConsolidatePVEInstancesPromotesCanonicalTokenMetadataFromStandalone(t *testing.T) {
	instances, changed := ConsolidatePVEInstances([]PVEInstance{
		{
			Name:        "homelab",
			ClusterName: "cluster-A",
		...
		},
		{
			Name:       "minipc-standalone",
			Host:       "10.0.0.5",
			TokenName:  "pulse@pve!canonical-token",
			TokenValue: "secret",
			Source:     "script",
		},
	})
	...
	if got := instances[0].TokenName; got != "pulse@pve!canonical-token" {
		t.Fatalf("TokenName = %q, want pulse@pve!canonical-token", got)
	}
	if got := instances[0].TokenValue; got != "secret" {
		t.Fatalf("TokenValue = %q, want secret", got)
	}
	if got := instances[0].Source; got != "script" {
		t.Fatalf("Source = %q, want script", got)
	}
}
