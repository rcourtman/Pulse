...
func aiProviderDefinitions() []AIProviderDefinition {
	return []AIProviderDefinition{
		...
		},
		{
			...
			Description:      "Gateway for models from multiple AI providers",
			Protocol:         AIProviderProtocolOpenAICompatible,
			DefaultModel:     "openai/gpt-4o-mini",
			DefaultBaseURL:   DefaultVercelBaseURL,
			APIKeyField:      "vercel_api_key",
			ConfiguredField:  "vercel_configured",
			ClearKeyField:    "clear_vercel_key",
			RequiresAPIKey:   true,
			UserConfigurable: true,
			Gateway:          true,
