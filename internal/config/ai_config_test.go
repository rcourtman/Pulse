...
func TestAIConfig_AreToolsDisabledForProvider(t *testing.T) {
	tests := []struct {
		name     string
		config   AIConfig
		provider string
		...
		{
			name:     "openai with custom base URL and tools disabled",
			config:   AIConfig{OpenAIBaseURL: "http://localhost:1234/v1", OpenAIToolsDisabled: true},
			provider: AIProviderOpenAI,
			expected: true,
		},
		{
			name:     "openai with custom base URL but tools enabled",
			config:   AIConfig{OpenAIBaseURL: "http://localhost:1234/v1", OpenAIToolsDisabled: false},
			provider: AIProviderOpenAI,
			expected: false,
		},
		{
			name:     "openai without custom base URL (standard OpenAI)",
			config:   AIConfig{OpenAIBaseURL: "", OpenAIToolsDisabled: true},
			provider: AIProviderOpenAI,
			expected: false, // should NOT disable for standard OpenAI
		},
		{
			name:     "anthropic provider",
			config:   AIConfig{OpenAIToolsDisabled: true},
			provider: AIProviderAnthropic,
			expected: false, // only applies to OpenAI with custom base URL
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.config.AreToolsDisabledForProvider(tt.provider)
			if result != tt.expected {
				t.Errorf("AreToolsDisabledForProvider(%q) = %v, want %v", tt.provider, result, tt.expected)
			}
		})
	}
}
