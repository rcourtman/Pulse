...
func TestAIConfig_HasProvider(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "vercel configured",
			config:   AIConfig{VercelAPIKey: "key"},
			provider: AIProviderVercel,
			expected: true,
		},
		{
...
func TestDefaultModelForProvider_UsesCanonicalProviderFallbacks(t *testing.T) {
	tests := []struct {
		...
		{provider: AIProviderVercel, expected: "vercel:openai/gpt-4o-mini"},
...
func TestAIConfig_GetBaseURLForProvider(t *testing.T) {
	...
	tests := []struct {
		...
		{AIProviderVercel, DefaultVercelBaseURL},
