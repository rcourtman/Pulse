(() => {
  ...
  function renderOverviewMetricStrip(totalCount, readyCount, checkingCount, unhealthyCount, suspendedCount) {
    return '<div class="account-overview-metrics account-overview-metrics-header"><div class="account-panel-kicker">Live posture</div><div class="account-metric-strip"><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Total</span><span class="account-stat-value">' + String(totalCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Ready now</span><span class="account-stat-value account-stat-healthy">' + String(readyCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Checking</span><span class="account-stat-value account-stat-checking">' + String(checkingCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Needs attention</span><span class="account-stat-value account-stat-unhealthy">' + String(unhealthyCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Suspended</span><span class="account-stat-value">' + String(suspendedCount) + "</span></div></div></div>";
  }
  ...
  function renderAccountOverviewSection(account) {
    ...
    var nextStepTitle = unhealthyCount > 0 ? "Start in workspaces" : checkingCount > 0 ? "Review pending checks" : suspendedCount > 0 ? "Next operator step" : "Next operator step";
    ...
    var consoleScopeCopy = account.kind === "msp" ? "Run client workspaces, account billing, and operator access from one place." : "Run hosted workspaces, account billing, and operator access from one place.";
    var overviewBriefStrip = '<div class="account-overview-brief-strip"><div class="account-overview-brief-point"><strong>Console role</strong><span>' + escapeHTML(consoleScopeCopy) + '</span></div><div class="account-overview-brief-point"><strong>Hosted path</strong><span>Use Workspaces for tenant operations and Team only when the operator roster needs to change.</span></div><div class="account-overview-brief-point"><strong>Commercial path</strong><span>Keep licenses, refunds, privacy, and self-hosted billing in Account services instead of mixing them into hosted work.</span></div></div>';
    return '<section class="account-content-panel account-content-panel-overview"><div class="account-stage-header account-stage-header-overview"><div><div class="account-panel-kicker">Overview</div><h3>Hosted posture</h3><p>Review hosted posture first, then move into the next operator desk.</p>' + renderSectionContextChips([
      String(totalCount) + " total",
      String(readyCount) + " ready",
      suspendedCount > 0 ? String(suspendedCount) + " suspended" : "Active fleet"
    ]) + renderOverviewMetricStrip(totalCount, readyCount, checkingCount, unhealthyCount, suspendedCount) + '</div></div><div class="account-command-deck"><div class="account-overview-main-column"><div class="account-overview-card account-overview-briefing-card"><div class="account-overview-lead"><div class="account-panel-kicker">Fleet posture</div><h3>' + escapeHTML(postureTitle) + "</h3><p>" + escapeHTML(postureCopy) + "</p>" + overviewBriefStrip + '</div><div class="account-overview-next-block"><div class="account-panel-kicker">Next move</div><h4>' + escapeHTML(nextStepTitle) + "</h4><p>" + escapeHTML(nextStepCopy) + "</p>" + nextStepChecklist + nextStepActions + '</div></div></div><div class="account-overview-side-column">' + renderAttentionPanel(workspaces) + "</div></div></section>";
  }
  ...
  function renderAuthenticatedPortalHTML(context) {
    ...
    var serviceNote = hosted ? "Hosted operations stay in Workspaces and Team. Use this desk only for self-hosted commercial requests." : "Use these account tools for self-hosted licenses, billing, refunds, and privacy actions.";
    ...
    return '<div class="portal-shell" data-shell-section="' + activeSection + '"><div class="portal-shell-layout">' + renderShellNavigation(accounts, context.bootstrap.support_email || "", activeSection) + '<div class="portal-shell-main">' + (accounts.length === 1 ? renderAccountContextStrip(accounts[0]) : "") + '<section class="portal-content-panel portal-content-panel-overview"><div id="accounts-root">' + hostedContent + '</div></section><section class="portal-content-panel portal-content-panel-services service-section" id="account-services-section"><div class="service-header"><div><div class="account-panel-kicker">Account services</div><h2>' + serviceHeading + "</h2>" + renderSectionContextChips([
      ...
      "4 desks"
    ]) + '</div><div class="service-note">' + serviceNote + '</div></div><div class="service-shell"><aside class="service-shell-sidebar"><div class="service-shell-sidebar-head"><div class="account-panel-kicker">Service navigator</div><h3>Self-hosted desks</h3><p>Pick one commercial workflow and keep it isolated from hosted workspace and team operations.</p></div><div class="service-action-list">' + renderServiceActionRow("open-manage-service", "Billing", "Manage subscriptions", "Billing desk", "Open Stripe for self-hosted plan, invoice, and payment changes.", "manage-service-panel", "manage-inline-email", ["Plan changes", "Invoices"]) + renderServiceActionRow("open-retrieve-service", "Licenses", "Retrieve licenses", "License desk", "Recover the latest active self-hosted license and invoice link.", "retrieve-service-panel", "retrieve-inline-email", ["Latest active license", "Invoice lookup"]) + renderServiceActionRow("open-refund-service", "Refunds", "Refund requests", "Refund desk", "Request a self-serve refund when the purchase is still eligible.", "refund-service-panel", "refund-inline-email", ["Eligibility check", "Revocation"]) + renderServiceActionRow("open-data-service", "Privacy", "Data and privacy", "Privacy desk", "Request export or deletion for commercial account data.", "data-service-panel", "data-export-email", ["Export", "Deletion"]) + '</div><div class="service-inline-support"><div class="account-panel-kicker">Commercial routing</div><h4>Keep this desk isolated</h4><p>Hosted workspace work stays in Workspaces and Team. Use Support only when a self-hosted billing, license, refund, or privacy desk cannot complete the request cleanly.</p><div class="service-inline-support-points"><div class="service-inline-support-point"><strong>Hosted stays hosted</strong><span>Tenant handoff, team access, and hosted billing do not belong in this commercial desk.</span></div><div class="service-inline-support-point"><strong>Escalate with context</strong><span>Bring the desk name and exact failed action if a commercial request needs support.</span></div></div><div class="service-inline-support-actions"><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="support">Open support desk</button></div></div></aside><div class="service-shell-main"><div class="service-detail-shell"><div class="service-panel service-panel-empty visible" id="service-panel-empty"><div class="account-panel-kicker">Desk brief</div><h3>Choose a desk to begin</h3><p>Use one self-hosted desk at a time. Each desk verifies commercial identity first, then keeps billing, license, refund, or privacy work contained in one place.</p><div class="service-empty-shell"><div class="service-empty-intro"><div class="service-empty-intro-item"><strong>One request</strong><span>Keep a single commercial task active instead of bouncing across desks.</span></div><div class="service-empty-intro-item"><strong>Identity first</strong><span>Verification happens before any sensitive account action opens.</span></div><div class="service-empty-intro-item"><strong>Escalate fast</strong><span>Route unusual billing, refund, privacy, or license failures to support from this desk.</span></div></div><div class="service-empty-primary"><div class="service-empty-section service-empty-section-compact"><div class="service-empty-column-title">Workflow map</div><div class="service-empty-flow-list"><div class="service-empty-flow"><strong>Billing desk</strong><span>Stripe customer portal access for invoices, payment methods, and plan changes.</span></div><div class="service-empty-flow"><strong>License desk</strong><span>Recover the latest active self-hosted license and the matching invoice link.</span></div><div class="service-empty-flow"><strong>Refund desk</strong><span>Check eligibility before revoking active commercial access.</span></div><div class="service-empty-flow"><strong>Privacy desk</strong><span>Request export or deletion without leaving Pulse Account.</span></div></div></div><div class="service-empty-section service-empty-section-compact"><div class="service-empty-column-title">Before you start</div><div class="service-empty-points service-empty-points-stack"><div class="service-empty-point"><strong>Verify first</strong><span>Each desk confirms the commercial email before opening sensitive actions.</span></div><div class="service-empty-point"><strong>Stay focused</strong><span>Keep one commercial request in flight instead of bouncing between desks.</span></div></div></div></div><div class="service-empty-side"><div class="service-empty-section service-empty-section-support"><div class="service-empty-column-title">Escalation</div><div class="service-empty-checklist"><div class="service-empty-check"><strong>Escalate quickly</strong><span>If billing, license, refund, or privacy work does not behave as expected, escalate from this desk.</span></div></div><div class="service-empty-support">Need help with billing, refund, privacy, or license requests? <a class="portal-support-link" href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a></div></div></div></div></div><div class="service-panel" id="manage-service-panel"><div id="manage-service-root"></div></div><div class="service-panel" id="retrieve-service-panel"><div id="retrieve-service-root"></div></div><div class="service-panel" id="refund-service-panel"><div id="refund-service-root"></div></div><div class="service-panel" id="data-service-panel"><h3>Data and privacy</h3><p>Request export or deletion of the commercial data tied to an email address. Payment data held directly by Stripe still requires support handling.</p><div class="subsection"><div id="data-export-root"></div></div><div class="subsection"><div id="data-delete-root"></div></div><div class="helper-text">Payment-card data stays with Stripe. For Stripe deletion support, contact <a href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a>.</div></div></div></div></div></section><section class="portal-content-panel portal-content-panel-support">' + renderSupportSection(context) + "</section></div></div></div>";
  }
