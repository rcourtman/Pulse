(() => {
  ...
  function renderAttentionPanel(workspaces) {
    ...
    if (!attention.length) {
      return '<div class="overview-side-card overview-side-card-stable"><div class="account-panel-kicker">Attention</div><h4>' + escapeHTML(suspendedCount > 0 ? "No active blockers" : "Fleet is clear") + "</h4><p>" + escapeHTML(
        suspendedCount > 0 ? "Active hosted workspaces are healthy. Suspended workspaces stay parked until you resume them." : "Every active hosted workspace currently reports a healthy status."
      ) + '</p><div class="overview-stable-list"><div class="overview-stable-item"><strong>Healthy now</strong><span>' + escapeHTML(
        suspendedCount > 0 ? "Active hosted workspaces are clear for routine use." : "All active hosted workspaces are clear for routine use."
      ) + "</span></div>" + (suspendedCount > 0 ? '<div class="overview-stable-item"><strong>Suspended stays parked</strong><span>' + escapeHTML(String(suspendedCount) + " workspace" + (suspendedCount === 1 ? " is" : "s are") + " suspended and intentionally out of day-to-day use.") + "</span></div>" : "") + '<div class="overview-stable-item"><strong>Use Team only for change</strong><span>Keep roster edits explicit instead of mixing them into normal workspace work.</span></div><div class="overview-stable-item"><strong>Keep billing separate</strong><span>Use account services only when the task is commercial, not operational.</span></div></div></div>';
    }
    ...
    return '<div class="overview-side-card"><div class="account-panel-kicker">Attention</div><h4>Needs review</h4><p>These workspaces should be checked before you treat the workspace list as fully healthy.</p><div class="overview-alert-list">' + items + "</div></div>";
  }
  function renderOverviewMetricStrip(totalCount, readyCount, checkingCount, unhealthyCount, suspendedCount) {
    return '<div class="account-overview-metrics account-overview-metrics-header"><div class="account-panel-kicker">Live status</div><div class="account-metric-strip"><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Total</span><span class="account-stat-value">' + String(totalCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Ready now</span><span class="account-stat-value account-stat-healthy">' + String(readyCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Checking</span><span class="account-stat-value account-stat-checking">' + String(checkingCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Needs attention</span><span class="account-stat-value account-stat-unhealthy">' + String(unhealthyCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Suspended</span><span class="account-stat-value">' + String(suspendedCount) + "</span></div></div></div>";
  }
  ...
  function renderAccountOverviewSection(account) {
    ...
    var suspendedCount = countWorkspacesByState(workspaces, "suspended");
    var postureTitle = unhealthyCount > 0 ? "Needs review" : checkingCount > 0 ? "Still settling" : suspendedCount > 0 ? "Active workspaces are stable" : "Workspaces are stable";
    var postureCopy = unhealthyCount > 0 ? "One or more workspaces still need attention before this account looks healthy." : checkingCount > 0 ? "Most workspaces look healthy, but some are still waiting on a completed health check." : suspendedCount > 0 ? "Active workspaces are healthy while suspended workspaces stay parked until you resume them." : "All workspaces are healthy and ready for routine use.";
    var nextStepTitle = unhealthyCount > 0 ? "Start in workspaces" : checkingCount > 0 ? "Review pending checks" : suspendedCount > 0 ? "Next step" : "Next step";
    ...
    var nextStepChecklist = unhealthyCount > 0 ? '<div class="overview-next-checklist"><div class="overview-next-check"><strong>1. Review attention items</strong><span>Open the fleet and inspect any workspace marked as checking or needs attention.</span></div><div class="overview-next-check"><strong>2. Resolve access blockers</strong><span>Use Team if the right people are not already attached to the hosted account.</span></div><div class="overview-next-check"><strong>3. Escalate billing separately</strong><span>Keep account billing or self-hosted license work out of the workspace review flow.</span></div></div>' : checkingCount > 0 ? '<div class="overview-next-checklist"><div class="overview-next-check"><strong>1. Verify pending checks</strong><span>Open the workspaces still settling and confirm they are safe to operate.</span></div><div class="overview-next-check"><strong>2. Keep access deliberate</strong><span>Change Team only if a pending workspace needs a different mix of access.</span></div><div class="overview-next-check"><strong>3. Keep commercial work separate</strong><span>Use account services or billing only when the workspace state is already understood.</span></div></div>' : '<div class="overview-next-checklist"><div class="overview-next-check"><strong>1. Open a workspace for the next operational task</strong><span>Move into Workspaces when you are ready to do hosted work.</span></div><div class="overview-next-check"><strong>2. Change access in Team only</strong><span>Keep roster changes explicit instead of mixing them into routine workspace work.</span></div><div class="overview-next-check"><strong>3. Keep billing and privacy separate</strong><span>Licenses, refunds, privacy, and self-hosted billing stay in their own section.</span></div></div>';
    ...
    var accountScopeCopy = account.kind === "msp" ? "Manage client workspaces, billing, and team access from one place." : "Manage hosted workspaces, billing, and team access from one place.";
    var overviewBriefStrip = '<div class="account-overview-brief-strip"><div class="account-overview-brief-point account-overview-brief-point-wide"><strong>Account scope</strong><span>' + escapeHTML(accountScopeCopy) + '</span></div><div class="account-overview-brief-point"><strong>Hosted path</strong><span>Use Workspaces for tenant access and lifecycle changes, and Team only when access needs to change.</span></div><div class="account-overview-brief-point"><strong>Commercial path</strong><span>Keep licenses, refunds, privacy, and self-hosted billing in Account services instead of mixing them into hosted work.</span></div></div>';
    return '<section class="account-content-panel account-content-panel-overview"><div class="account-stage-header account-stage-header-overview"><div><div class="account-panel-kicker">Overview</div><h3>Workspace status</h3><p>Review workspace status first, then move into the next section.</p>' + renderSectionContextChips([
      String(totalCount) + " total",
      String(readyCount) + " ready",
      suspendedCount > 0 ? String(suspendedCount) + " suspended" : "Active fleet"
    ]) + renderOverviewMetricStrip(totalCount, readyCount, checkingCount, unhealthyCount, suspendedCount) + '</div></div><div class="account-command-deck"><div class="account-overview-main-column"><div class="account-overview-card account-overview-briefing-card"><div class="account-overview-lead"><div class="account-panel-kicker">Workspace summary</div><h3>' + escapeHTML(postureTitle) + "</h3><p>" + escapeHTML(postureCopy) + "</p>" + overviewBriefStrip + '</div><div class="account-overview-next-block"><div class="account-panel-kicker">Next move</div><h4>' + escapeHTML(nextStepTitle) + "</h4><p>" + escapeHTML(nextStepCopy) + "</p>" + nextStepChecklist + nextStepActions + '</div></div></div><div class="account-overview-side-column">' + renderAttentionPanel(workspaces) + "</div></div></section>";
  }
  function renderAccountWorkspaceSection(account, accountAPIBasePath) {
    ...
    if (account.can_manage) {
      ...
      if (account.has_billing) {
        workspaceHeaderActions += '<button type="button" class="btn-secondary btn-compact" data-action="open-billing" data-account-id="' + escapeAttr(account.id) + '">Manage billing</button>';
      }
      workspaceHeaderActions += '<button type="button" class="btn-secondary btn-compact" data-action="toggle-team" data-account-id="' + escapeAttr(account.id) + '" data-shell-target="team">Manage team</button>';
  ...
  function renderAccountTeamSection(account) {
    var accessPolicy = '<div class="team-policy-panel"><div class="team-panel-heading"><h4>Access model</h4><p>Assign the smallest role that still lets someone do the work they own on this account.</p></div><div class="team-policy-list"><div class="team-policy-row"><strong>Owner</strong><span>Billing, team access, and full hosted control.</span></div><div class="team-policy-row"><strong>Admin</strong><span>Hosted workspace control plus billing for the account.</span></div><div class="team-policy-row"><strong>Tech</strong><span>Workspace control without billing ownership.</span></div><div class="team-policy-row"><strong>Read-only</strong><span>State review and verification without control-plane changes.</span></div></div></div>';
    var reviewDesk = '<div class="team-review-shell"><div class="team-review-strip"><div class="team-panel-heading team-panel-heading-tight"><div class="account-panel-kicker">Access review</div><h4>Keep access disciplined</h4><p>Use the roster as a controlled access list, not a dumping ground for vague shared access.</p></div><div class="team-review-grid"><div class="team-review-card"><strong>Owners stay rare</strong><span>Reserve Owner for billing, team, and full hosted control. Default to Admin, Tech, or Read-only first.</span></div><div class="team-review-card"><strong>Keep access narrow</strong><span>Use Tech for workspace control and Read-only for verification instead of handing out broader access.</span></div><div class="team-review-card"><strong>Remove stale access fast</strong><span>If someone no longer owns the work, remove them instead of leaving dormant access attached to the account.</span></div></div></div></div>';
    return '<section class="account-content-panel account-content-panel-team"><section class="team-management-panel team-section team-section-shell" id="team-section-' + escapeAttr(account.id) + '" data-actor-role="' + escapeAttr(account.role) + '"><div class="team-management-header"><div><div class="account-panel-kicker">Team management</div><h3>Account access</h3><p>Owners govern billing and access. Admins and techs keep hosted work moving day to day.</p>' + renderSectionContextChips([
      account.can_manage ? "Managed roster" : "View only",
      "Least privilege",
      "Hosted access"
    ]) + '</div><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="workspaces">Close team view</button></div><div class="team-management-stats" id="team-stats-' + escapeAttr(account.id) + '"></div><div class="team-management-grid"><div class="team-roster-column"><div class="team-roster"><div class="team-panel-heading"><h4>People on this account</h4><p>Keep the roster small and role assignment explicit. The people listed here are the ones who can manage these workspaces.</p></div><div class="team-roster-list" id="team-list-' + escapeAttr(account.id) + '"><div class="team-list-message">Loading\u2026</div></div></div>' + reviewDesk + '</div><div class="team-side-column"><div class="team-operations-panel"><div class="team-panel-heading team-panel-heading-tight"><div class="account-panel-kicker">Access controls</div><h4>Invite and role policy</h4><p>Keep the roster deliberate. Invite the smallest role first, then tighten access as responsibilities become clearer.</p></div><div class="team-operations-grid"><div class="team-invite-panel"><div class="team-panel-heading"><h4>Invite someone new</h4><p>Add another person with the minimum role they need for this account.</p></div><div class="team-invite"><div><label for="invite-email-' + escapeAttr(account.id) + '">Email</label><input type="email" id="invite-email-' + escapeAttr(account.id) + '" placeholder="user@example.com" autocomplete="off"></div><div><label for="invite-role-' + escapeAttr(account.id) + '">Role</label><select id="invite-role-' + escapeAttr(account.id) + '"><option value="admin">Admin</option><option value="tech">Tech</option><option value="read_only">Read-only</option></select></div><button type="button" class="btn-primary btn-compact" data-action="invite-member" data-account-id="' + escapeAttr(account.id) + '">Invite</button></div></div>' + accessPolicy + "</div></div></div></div></section></section>";
  }
  function renderSupportSection(context) {
    return '<section class="portal-support-panel"><div class="account-panel-kicker">Support</div><h2>Support</h2><p>Use this section when hosted access looks wrong, billing behaves unexpectedly, or you need help with commercial requests.</p>' + renderSectionContextChips(["Hosted issues", "Commercial requests", context.bootstrap.support_email ? "Email" : "Support"]) + '<div class="portal-support-brief-strip"><div class="portal-support-brief-card"><strong>Hosted path</strong><span>Workspace access, team control, tenant handoff, and hosted billing stay on the hosted account route.</span></div><div class="portal-support-brief-card"><strong>Commercial path</strong><span>Self-hosted billing, licenses, refunds, and privacy requests stay in Account services until escalation is needed.</span></div><div class="portal-support-brief-card"><strong>Escalate with context</strong><span>Include the exact account, workspace, section, and failed action so support can continue the same path immediately.</span></div></div><div class="portal-support-layout"><div class="portal-support-route-grid"><div class="portal-support-route-card"><div class="account-panel-kicker">Hosted account</div><h3>Hosted support</h3><p>Use this route when tenant handoff, workspace access, team control, or hosted billing looks wrong.</p><div class="portal-support-points"><div class="portal-support-point"><strong>Route here for hosted issues</strong><span>Access, handoff, team, and hosted billing all belong on the hosted account path.</span></div><div class="portal-support-point"><strong>Keep the account context intact</strong><span>Include the account, workspace, and action that failed so support can pick up the same issue quickly.</span></div></div><div class="portal-support-actions"><a class="portal-support-link" href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a></div></div><div class="portal-support-route-card"><div class="account-panel-kicker">Commercial</div><h3>Commercial requests</h3><p>Self-hosted subscriptions, license recovery, refunds, and privacy requests all route through Account services first.</p><div class="portal-support-points"><div class="portal-support-point"><strong>Start in Account services</strong><span>Use Billing, Licenses, Refunds, or Privacy before escalating a commercial issue.</span></div><div class="portal-support-point"><strong>Escalate from the same path</strong><span>Keep the request in one place instead of splitting it between billing and account sections.</span></div></div><div class="portal-support-actions"><button type="button" class="btn-secondary" data-shell-action="activate-section" data-shell-section="services">Open account services</button></div></div></div><div class="portal-support-runbook"><div class="account-panel-kicker">Escalation guide</div><h3>Choose the right path</h3><p>Keep hosted workspace issues, commercial requests, and pure support escalation on their own paths so the next person does not have to reconstruct the account state.</p><div class="portal-support-runbook-brief"><div class="portal-support-runbook-brief-card"><strong>Confirm scope</strong><span>Decide first whether the issue is a hosted workspace problem, a commercial self-service request, or direct support escalation.</span></div><div class="portal-support-runbook-brief-card"><strong>Keep paths separate</strong><span>Workspace and team problems stay out of billing, refund, privacy, and license work.</span></div><div class="portal-support-runbook-brief-card"><strong>Escalate with facts</strong><span>Bring the exact account, workspace, section, and failed action so support starts with the same state you saw.</span></div></div><div class="portal-support-runbook-grid"><div class="portal-support-runbook-section"><div class="portal-support-runbook-section-title">Route checklist</div><div class="portal-support-runbook-list"><div class="portal-support-runbook-step"><strong>1. Confirm the scope</strong><span>Decide whether the problem is a hosted workspace issue, a commercial self-service request, or direct support escalation.</span></div><div class="portal-support-runbook-step"><strong>2. Keep hosted and commercial separate</strong><span>Workspace and team problems stay in their own sections. Billing, license, refund, and privacy work stay in Account services.</span></div><div class="portal-support-runbook-step"><strong>3. Escalate with context</strong><span>Include the account, workspace, and exact failed action so the escalation path starts with the same facts you saw.</span></div></div></div><div class="portal-support-runbook-section"><div class="portal-support-runbook-section-title">Escalation packet</div><div class="portal-support-handoff-note"><strong>Include in the escalation</strong><span>Account name, workspace name if relevant, the section you were in, the exact button or request that failed, and whether the issue was hosted or commercial.</span></div></div></div></div></div></section>';
  }
  ...
  function renderAuthenticatedPortalHTML(context) {
    ...
    var serviceNote = hosted ? "Hosted workspace changes stay in Workspaces and Team. Use this area only for self-hosted commercial requests." : "Use these account tools for self-hosted licenses, billing, refunds, and privacy actions.";
    ...
    return '<div class="portal-shell" data-shell-section="' + activeSection + '"><div class="portal-shell-layout">' + renderShellNavigation(accounts, context.bootstrap.support_email || "", activeSection) + '<div class="portal-shell-main">' + (accounts.length === 1 ? renderAccountContextStrip(accounts[0]) : "") + '<section class="portal-content-panel portal-content-panel-overview"><div id="accounts-root">' + hostedContent + '</div></section><section class="portal-content-panel portal-content-panel-services service-section" id="account-services-section"><div class="service-header"><div><div class="account-panel-kicker">Account services</div><h2>' + serviceHeading + "</h2>" + renderSectionContextChips([
      hosted ? "Self-hosted only" : "Commercial",
      "4 tools"
    ]) + '</div><div class="service-note">' + serviceNote + '</div></div><div class="service-shell"><aside class="service-shell-sidebar"><div class="service-shell-sidebar-head"><div class="account-panel-kicker">Service navigator</div><h3>Self-hosted tools</h3><p>Pick one commercial request and keep it separate from hosted workspaces and team changes.</p></div><div class="service-action-list">' + renderServiceActionRow("open-manage-service", "Billing", "Manage subscriptions", "Billing", "Open Stripe for self-hosted plan, invoice, and payment changes.", "manage-service-panel", "manage-inline-email", ["Plan changes", "Invoices"]) + renderServiceActionRow("open-retrieve-service", "Licenses", "Retrieve licenses", "Licenses", "Recover the latest active self-hosted license and invoice link.", "retrieve-service-panel", "retrieve-inline-email", ["Latest active license", "Invoice lookup"]) + renderServiceActionRow("open-refund-service", "Refunds", "Refund requests", "Refunds", "Request a self-serve refund when the purchase is still eligible.", "refund-service-panel", "refund-inline-email", ["Eligibility check", "Revocation"]) + renderServiceActionRow("open-data-service", "Privacy", "Data and privacy", "Privacy", "Request export or deletion for commercial account data.", "data-service-panel", "data-export-email", ["Export", "Deletion"]) + '</div><div class="service-inline-support"><div class="account-panel-kicker">Commercial routing</div><h4>Keep this request separate</h4><p>Hosted workspace work stays in Workspaces and Team. Use Support only when a self-hosted billing, license, refund, or privacy request cannot complete cleanly.</p><div class="service-inline-support-points"><div class="service-inline-support-point"><strong>Hosted stays hosted</strong><span>Tenant handoff, team access, and hosted billing do not belong in this commercial section.</span></div><div class="service-inline-support-point"><strong>Escalate with context</strong><span>Bring the service name and exact failed action if a commercial request needs support.</span></div></div><div class="service-inline-support-actions"><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="support">Open support</button></div></div></aside><div class="service-shell-main"><div class="service-detail-shell"><div class="service-panel service-panel-empty visible" id="service-panel-empty"><div class="account-panel-kicker">Service brief</div><h3>Choose a service to begin</h3><p>Use one self-hosted request at a time. Each service verifies commercial identity first, then keeps billing, license, refund, or privacy work contained in one place.</p><div class="service-empty-shell"><div class="service-empty-primary"><div class="service-empty-section service-empty-section-compact"><div class="service-empty-column-title">What each tool does</div><div class="service-empty-flow-list"><div class="service-empty-flow"><strong>Billing</strong><span>Stripe customer portal access for invoices, payment methods, and plan changes.</span></div><div class="service-empty-flow"><strong>Licenses</strong><span>Recover the latest active self-hosted license and the matching invoice link.</span></div><div class="service-empty-flow"><strong>Refunds</strong><span>Check eligibility before revoking active commercial access.</span></div><div class="service-empty-flow"><strong>Privacy</strong><span>Request export or deletion without leaving Pulse Account.</span></div></div></div><div class="service-empty-section service-empty-section-compact"><div class="service-empty-column-title">Before you start</div><div class="service-empty-points service-empty-points-stack"><div class="service-empty-point"><strong>One request</strong><span>Keep a single commercial task active instead of bouncing across sections.</span></div><div class="service-empty-point"><strong>Identity first</strong><span>Verification happens before any sensitive account action opens.</span></div><div class="service-empty-point"><strong>Stay focused</strong><span>Keep one commercial request in flight instead of bouncing between services.</span></div></div></div></div><div class="service-empty-side"><div class="service-empty-section service-empty-section-support"><div class="service-empty-column-title">Escalation</div><div class="service-empty-checklist"><div class="service-empty-check"><strong>Escalate quickly</strong><span>If billing, licenses, refunds, or privacy do not behave as expected, escalate from this section.</span></div><div class="service-empty-check"><strong>Commercial packet</strong><span>Bring the service name, commercial email, and the exact failed action so support starts with the same request state.</span></div></div><div class="service-empty-support">Need help with billing, refund, privacy, or license requests? <a class="portal-support-link" href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a></div><div class="service-empty-actions"><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="support">Open support</button></div></div></div></div></div><div class="service-panel" id="manage-service-panel"><div id="manage-service-root"></div></div><div class="service-panel" id="retrieve-service-panel"><div id="retrieve-service-root"></div></div><div class="service-panel" id="refund-service-panel"><div id="refund-service-root"></div></div><div class="service-panel" id="data-service-panel"><h3>Data and privacy</h3><p>Request export or deletion of the commercial data tied to an email address. Payment data held directly by Stripe still requires support handling.</p><div class="subsection"><div id="data-export-root"></div></div><div class="subsection"><div id="data-delete-root"></div></div><div class="helper-text">Payment-card data stays with Stripe. For Stripe deletion support, contact <a href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a>.</div></div></div></div></div></section><section class="portal-content-panel portal-content-panel-support">' + renderSupportSection(context) + "</section></div></div></div>";
  }
