(() => {
  ...
  function renderAttentionPanel(workspaces) {
    ...
    if (!attention.length) {
      return '<div class="overview-side-card overview-side-card-stable"><div class="account-panel-kicker">Attention</div><h4>' + escapeHTML(suspendedCount > 0 ? "No active blockers" : "Fleet is clear") + "</h4><p>" + escapeHTML(
        suspendedCount > 0 ? "Active workspaces are healthy. Suspended workspaces stay parked until you resume them." : "Every active workspace currently reports a healthy status."
      ) + '</p><div class="overview-stable-list"><div class="overview-stable-item"><strong>Healthy now</strong><span>' + escapeHTML(
        suspendedCount > 0 ? "Active workspaces are clear for routine use." : "All active workspaces are clear for routine use."
  ...
  function renderAccountOverviewSection(account) {
    ...
    var accountScopeCopy = account.kind === "msp" ? "Manage client workspaces, billing, and team access from one place." : "Manage workspaces, billing, and team access from one place.";
    var overviewBriefStrip = '<div class="account-overview-brief-strip"><div class="account-overview-brief-point account-overview-brief-point-wide"><strong>Account scope</strong><span>' + escapeHTML(accountScopeCopy) + '</span></div><div class="account-overview-brief-point"><strong>Workspace path</strong><span>Use Workspaces for tenant access and lifecycle changes, and Team only when access needs to change.</span></div><div class="account-overview-brief-point"><strong>Commercial path</strong><span>Keep licenses, refunds, privacy, and self-hosted billing in Account services instead of mixing them into workspace work.</span></div></div>';
