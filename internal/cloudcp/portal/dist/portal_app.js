(() => {
  ...
  function renderAccountOverviewSection(account) {
    ...
    return '<section class="account-content-panel account-content-panel-overview"><div class="account-stage-header account-stage-header-overview"><div><div class="account-panel-kicker">Overview</div><h3>Hosted status</h3><p>Review hosted status first, then move into the next section.</p>' + renderSectionContextChips([
      String(totalCount) + " total",
      String(readyCount) + " ready",
      suspendedCount > 0 ? String(suspendedCount) + " suspended" : "Active fleet"
    ]) + renderOverviewMetricStrip(totalCount, readyCount, checkingCount, unhealthyCount, suspendedCount) + '</div></div><div class="account-command-deck"><div class="account-overview-main-column"><div class="account-overview-card account-overview-briefing-card"><div class="account-overview-lead"><div class="account-panel-kicker">Fleet status</div><h3>' + escapeHTML(postureTitle) + "</h3><p>" + escapeHTML(postureCopy) + "</p>" + overviewBriefStrip + '</div><div class="account-overview-next-block"><div class="account-panel-kicker">Next move</div><h4>' + escapeHTML(nextStepTitle) + "</h4><p>" + escapeHTML(nextStepCopy) + "</p>" + nextStepChecklist + nextStepActions + '</div></div></div><div class="account-overview-side-column">' + renderAttentionPanel(workspaces) + "</div></div></section>";
  }
