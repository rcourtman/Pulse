(() => {
  ...
  function renderShellNavigation(accounts, supportEmail, activeSection) {
    ...
    var readyWorkspaces2 = countReadyWorkspaces(workspaces);
  ...
  function readyWorkspaces(workspaces) {
    var results = [];
    for (var i = 0; i < workspaces.length; i += 1) {
      if (String(workspaces[i].state || "") === "active" && workspaceHealthState(workspaces[i]) === "healthy") {
        results.push(workspaces[i]);
  ...
  function renderOverviewAttentionCard(workspaces) {
    var attention = attentionWorkspaces(workspaces);
    if (!attention.length) {
      return '<article class="overview-task-card"><div class="account-panel-kicker">Needs attention</div><h4>Nothing urgent</h4><p>No active workspace is currently asking for review.</p><div class="overview-task-list"><div class="overview-task-item"><strong>Healthy now</strong><span>Active workspaces look clear for routine use.</span></div><div class="overview-task-item"><strong>Suspended stays parked</strong><span>Suspended workspaces stay out of the way until you deliberately resume them.</span></div></div></article>';
    }
    return '<article class="overview-task-card overview-task-card-attention"><div class="account-panel-kicker">Needs attention</div><h4>Review these first</h4><p>These workspaces still need a human check before you treat the account as settled.</p><div class="overview-task-list">' + attention.slice(0, 3).map(function(workspace) {
      return '<div class="overview-task-item"><strong>' + escapeHTML(workspace.display_name) + "</strong><span>" + escapeHTML(workspaceStatusCopy(workspace)) + "</span></div>";
    }).join("") + "</div></article>";
  }
  function renderOverviewReadyCard(account, workspaces, accountAPIBasePath) {
    var ready = readyWorkspaces(workspaces);
    if (!ready.length) {
      return '<article class="overview-task-card"><div class="account-panel-kicker">Ready</div><h4>No workspace is ready yet</h4><p>Use Workspaces to review current state before you start routine work.</p></article>';
    }
    return '<article class="overview-task-card"><div class="account-panel-kicker">Ready</div><h4>Open and work</h4><p>These workspaces are active and healthy right now.</p><div class="overview-task-list">' + ready.slice(0, 3).map(function(workspace) {
      return '<div class="overview-task-item overview-task-item-action"><div class="overview-task-copy"><strong>' + escapeHTML(workspace.display_name) + "</strong><span>" + escapeHTML(workspaceRowNote(workspace)) + "</span></div>" + renderWorkspaceHandoffForm(account.id, workspace.id, accountAPIBasePath, "Open workspace") + "</div>";
    }).join("") + "</div></article>";
  }
  function renderOverviewNextActionCard(account, workspaces, accountAPIBasePath) {
    var attention = attentionWorkspaces(workspaces);
    var ready = readyWorkspaces(workspaces);
    var primaryAction = "";
    var secondaryAction = "";
    ...
    if (attention.length) {
      ...
      secondaryAction = account.can_manage ? '<button class="btn-secondary btn-compact" type="button" data-shell-action="activate-section" data-shell-section="access">Review access</button>' : "";
  ...
  function renderAccountOverviewSection(account, accountAPIBasePath) {
    ...
    ]) + '</div></div><div class="overview-task-grid">' + renderOverviewAttentionCard(workspaces) + renderOverviewReadyCard(account, workspaces, accountAPIBasePath) + renderOverviewNextActionCard(account, workspaces, accountAPIBasePath) + '</div><div class="overview-task-summary-bar"><div class="overview-task-summary-pill"><strong>Healthy</strong><span>' + String(countWorkspacesByHealth(workspaces, "healthy")) + '</span></div><div class="overview-task-summary-pill"><strong>Checking</strong><span>' + String(checkingCount) + '</span></div><div class="overview-task-summary-pill"><strong>Needs attention</strong><span>' + String(unhealthyCount) + '</span></div><div class="overview-task-summary-pill"><strong>Suspended</strong><span>' + String(suspendedCount) + "</span></div></div></section>";
  }
  ...
  function renderHostedBillingCards(accounts) {
    ...
    if (!hostedBillingAccounts.length) {
      return '<div class="billing-task-card billing-task-card-muted"><div class="account-panel-kicker">Hosted billing</div><h3>No hosted billing attached</h3><p>Use the self-hosted billing tools below only if you are managing a self-hosted purchase.</p></div>';
    }
  ...
  function renderSupportSection(context) {
    return '<section class="portal-support-panel"><div class="account-panel-kicker">Support</div><h2>Escalation</h2><p>Come here only after the workspace, access, or billing path has stopped you.</p>' + renderSectionContextChips(["Escalation only", "Bring context", context.bootstrap.support_email ? "Email" : "Support"]) + '<div class="portal-support-layout"><div class="portal-support-route-grid"><div class="portal-support-route-card"><div class="account-panel-kicker">Hosted account</div><h3>Hosted problems</h3><p>Use support for hosted issues only after you have already tried the Workspaces or Access path.</p><div class="portal-support-points"><div class="portal-support-point"><strong>Use after the hosted path fails</strong><span>Workspace access, lifecycle, and account access should already be narrowed before you escalate.</span></div><div class="portal-support-point"><strong>Keep the account context intact</strong><span>Include the account, workspace, and action that failed so support can pick up the same issue quickly.</span></div></div><div class="portal-support-actions"><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="workspaces">Open workspaces</button><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="access">Open access</button><a class="portal-support-link" href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a></div></div><div class="portal-support-route-card"><div class="account-panel-kicker">Billing</div><h3>Billing and self-hosted issues</h3><p>Use this route only after hosted billing or the self-hosted billing tools have failed to complete the request cleanly.</p><div class="portal-support-points"><div class="portal-support-point"><strong>Start in Billing</strong><span>Hosted billing, licenses, refunds, and privacy should already be narrowed to one failed request.</span></div><div class="portal-support-point"><strong>Escalate from the same path</strong><span>Bring the billing tool or hosted billing action that failed instead of reopening the story from scratch.</span></div></div><div class="portal-support-actions"><button type="button" class="btn-secondary btn-compact" data-shell-action="activate-section" data-shell-section="billing">Open billing</button><a class="portal-support-link" href="mailto:' + escapeAttr(context.bootstrap.support_email || "") + '">' + escapeHTML(context.bootstrap.support_email || "") + '</a></div></div></div><div class="portal-support-runbook"><div class="account-panel-kicker">Escalation packet</div><h3>What to send</h3><p>Keep the packet short and concrete so support can pick up the exact failed path immediately.</p><div class="portal-support-runbook-list"><div class="portal-support-runbook-step"><strong>1. Name the path</strong><span>Say whether the failed path was Workspaces, Access, or Billing.</span></div><div class="portal-support-runbook-step"><strong>2. Name the account</strong><span>Include the hosted account and workspace when relevant, or the billing email for self-hosted work.</span></div><div class="portal-support-runbook-step"><strong>3. Name the action</strong><span>Include the exact button, form, or billing tool that failed and what happened next.</span></div></div></div></div></section>';
  }
  ...
  function renderAuthenticatedPortalHTML(context) {
    ...
    var billingNote = hosted ? "Hosted billing lives here when it applies. Self-hosted licenses, refunds, and privacy stay separate underneath it." : "Use this billing surface for self-hosted subscriptions, licenses, refunds, and privacy requests.";
