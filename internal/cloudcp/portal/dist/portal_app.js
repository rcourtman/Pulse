(() => {
  ...
  function renderAccountOverviewSection(account) {
    ...
    return '<section class="account-content-panel account-content-panel-overview"><div class="account-stage-header account-stage-header-overview"><div><div class="account-panel-kicker">Overview</div><h3>Hosted posture</h3><p>Review hosted posture first, then move into the next operator desk.</p></div></div><div class="account-command-deck"><div class="account-overview-card"><div class="account-overview-lead"><div class="account-panel-kicker">Fleet posture</div><h3>' + escapeHTML(postureTitle) + "</h3><p>" + escapeHTML(postureCopy) + '</p><div class="account-overview-callout">' + escapeHTML(
    ...
    ) + '</div></div><div class="account-metric-strip"><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Total</span><span class="account-stat-value">' + String(totalCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Ready now</span><span class="account-stat-value account-stat-healthy">' + String(readyCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Checking</span><span class="account-stat-value account-stat-checking">' + String(checkingCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Needs attention</span><span class="account-stat-value account-stat-unhealthy">' + String(unhealthyCount) + '</span></div><div class="account-stat-card account-stat-card-inline"><span class="account-stat-label">Suspended</span><span class="account-stat-value">' + String(suspendedCount) + '</span></div></div></div><div class="account-overview-secondary"><div class="overview-side-card overview-side-card-primary"><div class="account-panel-kicker">Next move</div><h4>' + escapeHTML(nextStepTitle) + "</h4><p>" + escapeHTML(nextStepCopy) + "</p>" + nextStepChecklist + nextStepActions + "</div>" + renderAttentionPanel(workspaces) + "</div></div></section>";
  }
  function renderAccountWorkspaceSection(account, accountAPIBasePath) {
    ...
    if (account.can_manage) {
      ...
      workspaceManagement = '<section class="workspace-management-panel" id="workspace-management-' + escapeAttr(account.id) + '"><div class="workspace-management-header"><div><div class="account-panel-kicker">Workspace management</div><h3>Workspace desk</h3><p>Select one workspace from the fleet to inspect posture, lifecycle, and the next explicit operator action.</p></div><button type="button" class="btn-secondary btn-compact" id="workspace-management-close-' + escapeAttr(account.id) + '" data-action="clear-workspace-selection" data-account-id="' + escapeAttr(account.id) + '">Close desk</button></div><div class="workspace-management-empty" id="workspace-management-empty-' + escapeAttr(account.id) + '"><div class="workspace-management-empty-copy">No workspace selected yet.</div><div class="workspace-management-empty-grid"><div class="workspace-management-empty-card"><div class="account-panel-kicker">Account actions</div><h4>Keep account-wide actions separate</h4><p>Billing, team, and workspace creation stay account-wide even when you are focused on one workspace.</p><div class="workspace-management-empty-actions">' + workspaceDeskActions + "</div>" + addWorkspaceForm + '</div><div class="workspace-management-empty-card workspace-management-empty-card-muted"><div class="account-panel-kicker">When you pick a workspace</div><div class="workspace-management-empty-checklist"><div class="workspace-management-empty-check"><strong>Inspect posture</strong><span>Load the workspace first, then decide whether it is routine work, review work, or a parked suspended system.</span></div><div class="workspace-management-empty-check"><strong>Confirm lifecycle</strong><span>Check whether the workspace is active, checking, failed, or suspended before taking the next action.</span></div><div class="workspace-management-empty-check"><strong>Then act deliberately</strong><span>Use the desk to manage one workspace at a time instead of mixing account-wide actions into the same flow.</span></div></div></div></div></div><div class="workspace-management-content" id="workspace-management-content-' + escapeAttr(account.id) + '" hidden><div class="workspace-management-meta" id="workspace-management-meta-' + escapeAttr(account.id) + '"></div><h4 id="workspace-management-title-' + escapeAttr(account.id) + '"></h4><p class="workspace-management-summary" id="workspace-management-summary-' + escapeAttr(account.id) + '"></p><div class="workspace-management-facts"><div class="workspace-management-fact"><span>Health</span><strong id="workspace-management-health-' + escapeAttr(account.id) + '"></strong></div><div class="workspace-management-fact"><span>Lifecycle</span><strong id="workspace-management-lifecycle-' + escapeAttr(account.id) + '"></strong></div><div class="workspace-management-fact"><span>Created</span><strong id="workspace-management-created-' + escapeAttr(account.id) + '"></strong></div></div><div class="workspace-management-guidance" id="workspace-management-guidance-' + escapeAttr(account.id) + '"></div><div class="workspace-management-actions"><button type="button" class="btn-danger" id="workspace-management-action-' + escapeAttr(account.id) + '" data-action="workspace-action" data-account-id="' + escapeAttr(account.id) + '">Manage workspace</button></div></div></section>';
    }
    ...
    return '<section class="account-content-panel account-content-panel-workspaces"><div class="account-stage-header"><div class="account-stage-header-row"><div><div class="account-panel-kicker">Workspace fleet</div><h3>Hosted fleet</h3><p>Use the fleet view to open workspaces, watch health posture, and keep management actions explicit.</p></div><div class="account-stage-header-actions">' + workspaceHeaderActions + '</div></div></div><div class="workspace-operations-shell"><div class="workspace-operations-main">' + workspaceHTML + '</div><div class="workspace-operations-detail">' + workspaceManagement + "</div></div></section>";
  }
  ...
  function renderAuthenticatedPortalHTML(context) {
    ...
    var serviceNote = hosted ? "Hosted operations stay in Workspaces and Team. Use this area only for self-hosted commercial tasks." : "Use these account tools for self-hosted licenses, billing, refunds, and privacy actions.";
